...
5. **NEW INFRA finding (CORRECTED post-test): subagents/workflows are BLOCKED at ~213K tokens — but the cause is the TOOL-DEFINITION CATALOG + system prompt, NOT the `CLAUDE.md` shim.** The v167 fan-out died at the 200K limit; I first blamed the shim and compacted it (640KB→29KB, a real *main-loop* hygiene win), **but a post-compaction subagent smoke-test STILL failed at ~213.9K (was ~213.8K — ≈unchanged), proving the shim was not the driver.** The ~213K is the large connected-MCP/deferred-tool catalog + system prompt = an **environment/config** issue. Real fix (STILL OPEN): trim connected MCP tools, and/or re-test in a fresh session (in case subagent context is session-start-snapshotted). (Finding 7.) ⚠️ The filename's "shim-context-limit" label is superseded by this correction.
...
## FINDING 7 — subagents/workflows BLOCKED at ~213K tokens — cause = the TOOL-DEFINITION CATALOG, not the shim (CORRECTED 2026-06-16 post-test)

The fan-out workflow for this audit **failed at launch — all 20 subagents returned "Prompt is too long · ~213,772 tokens (limit 200,000)"** before executing a step. The error attributes the bulk to "system prompt, tool definitions, and attachment content."

**My initial diagnosis was WRONG, and the test caught it.** I first blamed the `CLAUDE.md` shim (which had genuinely ballooned to ~640KB / ~160K tokens) and ran a shim-rebuild — compacting it to **~29KB / ~7K tokens** (a real *main-loop* hygiene win: the main loop loads the full shim every turn, so this is worth keeping regardless). **But a post-compaction subagent smoke-test STILL failed at ~213,925 tokens — essentially unchanged from the pre-compaction ~213,772 (Δ ≈ +153).** Removing ~150K tokens of CLAUDE.md moved the subagent total by zero. Therefore the shim is **not** the live driver of the subagent limit; the ~213K is **system prompt + tool definitions** — the large catalog of connected MCP servers + deferred tools (Claude_in_Chrome, Claude_Preview, ccd_*, headroom, telegram, scheduled-tasks, mcp-registry, …), whose schemas are injected into every subagent.

**Two hypotheses remain (one untested):** **(a)** the tool-definition catalog genuinely dominates (~200K) → only trimming connected MCP servers/tools fixes it; **(b)** subagent project-context is **snapshotted at session-start** (so this session's subagent still sees the pre-compaction 640KB shim, and a FRESH session would see the 29KB one and might pass). The Δ≈0 result is consistent with both, so a **fresh-session re-test is the cheap definitive check.** The magnitude argument leans toward (a): if the full 640KB shim were inside the 213K, the total would be ~300K+, not 213K — suggesting the full shim was never the bulk.

**Consequence:** workflows/subagents are currently **non-viable** for this vault in-session (main-loop Opus 1M-context still works — which is why this audit completed solo). **The real fix is environment-config, not vault-content:**

> **Rec (iii) [v162] — RE-SCOPED.** The shim-rebuild was EXECUTED (640KB→29KB) as **main-loop hygiene** and stands — but it is **NOT** the subagent fix. To restore subagent/workflow capability: **(a)** trim the connected MCP servers / deferred-tool set (the likely dominant cost), and/or **(b)** re-test workflows in a FRESH session (definitive test of the snapshot hypothesis). The shim compaction still follows the session-67/68 + hello-agents-Ch-9 discipline and reduces main-loop context-rot; it just doesn't solve this particular block.

**Honest note:** this is a diagnostic miss I made, which the smoke-test corrected — recorded here per the prime directive ("don't repeat the same mistake twice"). The headline + the shim banner + the `02b`/`06` notes were all corrected to match. ⚠️ The filename's "shim-context-limit INFRA finding" label predates this correction.

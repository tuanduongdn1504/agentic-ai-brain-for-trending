...
| 38a (N=2) | system_prompts_leaks v205 (`asgeirtj/system_prompts_leaks`) **NEW** — DIFFERENT repo/author from v21; the genre's **"legitimate-accountable" contrast pole** | **multi-tool-prompt-archive** | ~15 vendors / ~40 tools (broad; **Claude-heavy** — Opus 4.8 / Sonnet 5 / Fable 5 / Code / Design / Cowork / Mobile) | **Disclosed real individual** (Ásgeir Thor Johnson, Iceland — **NOT pseudonymous**) | **None** (CC0-1.0 public domain; no Patreon/crypto) | Undisclosed / crowd-sourced ("PRs Welcome"); only "Claude confirms authenticity" self-validation | Iceland (disclosed) |
...
>
> - **38a multi-tool-prompt-archive** — broad subject scope (multiple AI tools); system prompts as primary content; ~~pseudonymous author; commercial-derivative-funded monetization; opaque extraction~~. **N=2 at v205** (recorded, N-tally = audit bookkeeping, not self-incremented): (i) **system-prompts-leaks v21** (x1xhlol; 31 AI tools; pseudonymous / GPL-3.0 / 6-channel crypto+ZeroLeaks / one-time leak) + (ii) **system_prompts_leaks v205** (`asgeirtj`; ~40 tools; **disclosed real identity / CC0 / no monetization / continuously-updated / WaPo-covered** — the "legitimate-accountable" contrast pole). ⚠️ The v205 2nd instance **narrows the sub-variant definition**: the pseudonymity / commercial-funding / opaque-extraction attributes belong to the *x1xhlol instance*, NOT to 38a-the-sub-variant (which is just "broad multi-tool prompt archive"). This bounds candidates #36 (Pseudonymous-Researcher), #37 (Crypto-Donation-Funded), #40 (Derivative-Security-Service) to the x1xhlol instance — see the co-occurrence notes below.
...
- **Pattern #36 Pseudonymous Researcher Archetype (candidate v21):** the v21 x1xhlol 38a instance co-occurs with #36; 38b does NOT; **and the v205 asgeirtj 38a instance does NOT (disclosed real identity)** → #36 scope-narrows to the *x1xhlol instance*, NOT to the 38a sub-variant. **Pseudonymity is NOT required by the prompt-leak-archive genre.**
- **Pattern #37 Crypto-Donation-Funded Scale Path (candidate v21):** the v21 x1xhlol 38a instance co-occurs with #37; 38b does NOT; **and the v205 asgeirtj 38a instance does NOT (no monetization)** → #37 scope-narrows to the *x1xhlol instance*, NOT to the 38a sub-variant. **Monetization is NOT required by the genre.**
- **Pattern #40 Derivative-Security-Service Perverse-Incentive (candidate v21):** the v21 x1xhlol 38a instance co-occurs with #40 (ZeroLeaks); 38b does NOT; **and the v205 asgeirtj 38a instance does NOT** → #40 scope-narrows to the *x1xhlol instance*, NOT to the 38a sub-variant. *(#39 Controversial-License-Use likewise contrasts: x1xhlol = GPL-3.0-on-non-owned-content; asgeirtj = CC0 public-domain — the opposite license choice for the same extracted-not-owned content.)*
...
- **84c instance-arc (well beyond the retire threshold):** v71 → v72 DeepSeek-TUI → v73 cc-switch → v75 impeccable (14-harness CORPUS-RECORD) → v76 agent-skills-standard (sub-sub-mechanisms 84c.1 repo-stored / 84c.2 CLI-generates-native-formats-from-canonical-source) → **v168 ponytail** (clean instance-strengthening; NO mint). **v168 ponytail = a premium 84c "Provider-Agnostic-By-Design" instance**, 14 declared platforms, a **HYBRID mechanism strengthening BOTH** 84c.1 (6 repo-stored native rule copies — `.cursor`/`.windsurf`/`.clinerules`/`.agents`/`.github-copilot-instructions`/`.kiro` — enforced by a byte-for-byte drift-checker + a 7-rule-phrase-invariant CI gate) AND 84c.2 (`.openclaw/skills/` auto-generated from `skills/` via `build-openclaw-skills.js`, freshness-tested). The CI invariant-validation discipline is a more-rigorous take on multi-target alignment than v75's plain repo replicas — recorded as an OBSERVATION, not a mint. ponytail's distinct CONTENT (a code-minimalism methodology) is filed as a NEW Library-vocab §C standalone (registry `06`), NOT a new sub-mechanism here (the distribution structure is 84c; the content is the standalone). **v169 SkillSpector — explicitly NOT a 84c rule-distribution instance (anti-contamination note):** SkillSpector is a *scanner* with 3 pluggable LLM providers (OpenAI/Anthropic/NVIDIA) + 4 output formats — its only 84c relation is provider-agnostic-by-design on the **LLM-backend axis** (a scoped OBSERVATION, not the 84c.1/84c.2 native-rule-file-distribution mechanism). The v169 build workflow contaminated its candidate by importing ponytail's "14-platform cross-harness rule distribution / multi-format-output 84c, N=4→N=5" onto SkillSpector — REJECTED at synthesis. Do NOT count SkillSpector as a 84c.1/84c.2 instance.
...
**v162 agent-of-empires note — DECOMPOSES the held orchestration theme (session-multiplexer species → N=2):** AoE (`agent-of-empires/agent-of-empires` / `njbrake/agent-of-empires`, Nate Brake @ Mozilla.ai; Rust+tmux session manager — runs N coding agents in parallel, one per git-worktree, TUI + web dashboard/PWA, 13 agents) is the genuine **2nd instance of the "session-hosting multiplexer for AI coding agents (human-driven)" species** that v99 cmux anchored (cmux native-desktop/libghostty/Swift + AoE cross-platform-TUI+web/Rust+tmux — same core abstraction: host/multiplex parallel human-driven agent sessions; deltas = delivery/implementation variation, not a different KIND). So the **session-multiplexer species reaches N=2**, which REFINES the line above: the "AI-agent orchestration theme" is **less heterogeneous than held** — it **decomposes** into a **session-multiplexer species (cmux + AoE, N=2, PROMOTION-ELIGIBLE)** / **Paseo's agent-of-agents orchestration platform (N=1 — distinct: AoE is its *inverse*, a thing TO BE orchestrated via its HTTP API "integrates with OpenClaw," not an orchestrator; AoE lacks the `/paseo-*` agent-of-agents skill-suite recursion)** / **harness's team-architecture generator (adjacent)**. **DEFER the CONFIRM-or-split + rename decision to the overdue audit** (the v158 "don't generalize-the-definition-to-fit-the-2nd-instance" discipline — cmux's working label "Native-Desktop Terminal Multiplexer" encodes specifics AoE doesn't share; the audit decides generalize-to-delivery-agnostic [native-desktop / cross-platform-TUI+web sub-flavors] vs keep-as-distinct-sub-flavors). **0 new standalones; top-level count UNCHANGED at 46; CONFIRMED Library-vocab UNCHANGED at 10.** AoE ≠ v161 CodePilot's §C "Desktop GUI Client" standalone (CodePilot = single-agent visual front-end replacing the terminal; AoE = fleet multiplexer of normal terminal sessions). **ACP (Agent Client Protocol)** made architecturally-first-class by AoE (`acp-worker/` + structured-view UI) = a **Pattern #18 sub-mechanism B protocol-variant CANDIDATE** (the B1 MCP / B2 CDP / B3-reserved precedent), NOT a §C standalone and NOT corpus-first (recurs incidentally at v60 free-claude-code + goclaw + get-shit-done; AoE is merely the first to make it first-class) — OBSERVATION, promotion-eligible to a tracked B-variant at a 2nd ACP-first-class subject. (a) FAIL (US individual + Mozilla.ai-backed community OSS; Mozilla.ai raised-and-rejected as (a)-7 — not a vault substrate dependency; notable ≠ PASS, v139). §35 STAYS CLEAR (window {v160 GA, v161 GA, v162 GA} = 0 OG; 10 consecutive GA v153→v162). Streak GA:24 → **GA:25·OG:11 [7 ov]**.

**v163 ai-maestro note — COMPLETES the 2nd bucket (orchestration PLATFORM → N=2):** `23blocks-OS/ai-maestro` (Juan Pelaez, CTO & Founder 3Metas+23blocks, Boulder/Roma; TypeScript 87.8%; MIT; ≈713★/995c/v0.35.54) = a self-hosted **multi-vendor orchestration platform** managing Claude Code/Codex/Aider/Cursor/OpenClaw/Hermes/Droid from one Next.js dashboard, with **agent-to-agent messaging** (AMP) + persistent memory (CozoDB) + code-graph (ts-morph) + a multi-machine peer mesh + three home-grown open protocols (AMP/AID/AAP) + a Claude Code plugin (5 skills + 32 CLI scripts). It is the genuine **2nd instance of the v150 Paseo "Multi-Vendor Coding-Agent Orchestration Platform" standalone → that standalone reaches N=2** (registry `06` §F updated), which **COMPLETES the 2nd bucket of the v162 decomposition above**: the held theme now resolves into **session-multiplexer species (cmux + AoE, N=2)** + **orchestration-platform class (Paseo + ai-maestro, N=2)** + **harness team-generator (N=1 adjacent)** — *both* N=2 buckets PROMOTION-ELIGIBLE and DEFERRED to the overdue audit (which now owes two promote/rename calls). **Two sub-flavors recorded INSIDE the platform class** (8a/8b precedent): **(a) agent-of-agents-via-skill-suite** (Paseo `/paseo-*`, cross-DEVICE) / **(b) agent-to-agent-messaging-mesh** (ai-maestro AMP, multi-MACHINE + memory + code-graph). **CONFIRM-broad-class (drop Paseo's "via a skill suite" parenthetical = RENAME) vs keep-two-sub-flavors DEFERRED to the audit** (the v162/v158 "don't generalize-to-fit" discipline). **0 new §C standalones; top-level count UNCHANGED at 46; CONFIRMED Library-vocab UNCHANGED at 10.** Observations (NOT minted): **AMP agent-to-agent peer messaging** = the (b) sub-flavor + a candidate future standalone (DEFER; v162-ACP discipline; distinct from T4c operator-notification) · **AMP/AID/AAP home-grown protocol suite** = a Pattern #18 sub-mechanism-B protocol-variant CANDIDATE if a 2nd subject adopts (home-grown ≠ corpus-first) · **corpus-recursive capability absorption** — CozoDB memory ↔ v66 agentmemory/#85/v132 + ts-morph code-graph ↔ v70 codegraph; the platform ABSORBS prior-standalone capabilities as sub-features → NOT double-counted (components, not the product; a forming meta-observation) · T4c gateways N+1 (Slack/Discord/Email/WhatsApp) · #84 7-agent cross-tool · #57 5-skill Claude Code plugin (PRODUCT, not dev-exhaust — the v122 distinction inverted; NOT a 57k implementer) · #66 supply-chain (curl|bash + crypto-identity + 34-injection-screen) · NOT #18 #8 / NOT observability sub-archetype / NOT LV-C7 #22 / NOT the cmux/AoE session-multiplexer species. (a) FAIL (US/Italy founder; not a cultural-peer, not (a)-7; notable-founder ≠ PASS, v139). §35 STAYS CLEAR (window {v161 GA, v162 GA, v163 GA} = 0 OG; 11 consecutive GA v153→v163). Streak GA:25 → **GA:26·OG:11 [7 ov]**.
...
- **(a) Metering / quota / usage-logs** — numeric/dashboard surfacing of usage/cost/quota/logs. **N=4 at audit → N=5 at the v159 CodexBar ship → N=6 at the v165 lazyagent ship** (v89+v109+v157+v158+**v159 CodexBar**+**v165 lazyagent**; sub-archetype overall N=7 → N=8 → **N=11** → **N=12** [+v160 (c) +v164 (b) +v166 (b)]).
- **(b) Ambient / affective status-surface** — a reactive desktop-pet/avatar whose state tracks aggregate agent run-state as the *primary* display (affective signal, not a dashboard). **N=3 at audit → N=4 at the v164 oc-claw ship → N=5 at the v166 ai-agent-session-center ship** (v154+v155+v156+**v164 oc-claw**+**v166 ai-agent-session-center** [the first **web-delivered / 3D-robot / dashboard-hybrid** (b) instance — see its note]; sub-archetype overall **N=9 → N=10 → N=12** [v165 (a) intervenes at N=11]). **v167 audit (2026-06-16): "web-dashboard-hybrid" formalized as a delivery sub-note WITHIN (b) — NOT a split, NOT a new sub-flavor (the v160/v165 anti-inflation discipline); (b) now spans desktop-pet delivery [v154/v155/v156/v164] + web-dashboard-hybrid [v166 — run-state 3D-robot avatars primary, co-hosting dashboard panels + (c)-approval + light prompt-queue].**
- **(c) Attention / approval-interrupt-routing surface** — surfaces the *actionable* moments an agent needs a human (tool-call approval, input/follow-up, review, intervention), handles **approve/deny/answer in place**, and **routes focus back** to the originating terminal/IDE; characteristically **notch / Dynamic-Island-first** (compact-until-needed, not always-visible) + **SSH-forwarding** of remote sessions. Explicitly *"actionable focus interrupts ... rather than read-only dashboards."* **PROVISIONAL N=1** (v160 ping-island). CORPUS-FIRST; **minted at the v160 ping-island ship** (NOT manufactured — ping-island is functionally neither metering (a) nor an affective pet (b); the (a)/(b)-inside-sub-archetype precedent makes a 3rd sub-flavor the minimal-inflation home, NOT a new top-level standalone). Distinct from LV-C7 #22 (handling the agent's *own* permission prompts = pass-through, the v156 affordance; observe+route ≠ control config/accounts). External forming-genre corroboration (NOT in corpus, NOT counted): "Dynamic Island for AI coding agents" (CodeIsland / xisland / Vibe Island). **Promotion-eligible at a genuine in-corpus N=2.**

**v164 oc-claw post-audit ship (2026-06-15):** clean instance-strengthening of sub-flavor **(b) → N=4 / sub-archetype → N=10** (`rainnoon/oc-claw`, cross-platform **Tauri v2** desktop-pet that animates by aggregate agent run-state on the macOS notch / Windows taskbar; 7 agents — OpenClaw-first via JSONL polling + hooks/plugins; remote SSH; built at the KAON Hackathon; ≈288★/525c/v1.8.6/MIT). **0 new mints** — the v159 CodexBar shape (a clean instance of an existing sub-flavor mints nothing; its "firsts" — Tauri-stack pet / notch-placed pet / pet+token-metering blend — are stack/UI/blend, not structure). **0 new §C standalones; 0 new CONFIRMED Library-vocab (stays 10); top-level count UNCHANGED at 46.** (a) FAIL (`rainnoon` no disclosed identity → no handle-inferred cultural-peer, v139 / v160-ping-island discipline; KAON-Hackathon-origin = (d)/origin note, not an (a) signal). NOT (c) (animates by status + hover-detail, no actionable-interrupt routing / act-in-place — the notch is a shared UI-location, not a (c) marker). OBSERVES, NOT control → NOT LV-C7 #22 ("Tauri-but-NOT-LV-C7", the v158 line). Independent of v156 clawd-on-desk (different author/stack — Tauri vs Electron; not a fork). Observation: SSH-forwarding within the cluster → N=2 (v160 + v164). NOT a #52 claim (288★, age unverified §37.4).

**v165 lazyagent post-audit ship (2026-06-15):** clean instance-strengthening of sub-flavor **(a) → N=6 / sub-archetype → N=11** (`illegalstudio/lazyagent` / `nahime0`, a Go-core multi-interface monitor — Terminal UI + macOS menu-bar (Wails) + Bearer-token HTTP API + paid iOS companion — for 9 agents [Claude Code/Cursor/Codex/Grok CLI/Kilo/Kimi/Amp/pi/OpenCode]; real-time tokens/cost [model/cache-reuse/cache-creation] + `limits` rate/billing + deep session/subagent/tool/prompt inspection; MIT; ≈165★/373c/v0.12.5). **0 mints at the sub-archetype layer** — it is the broadest multi-interface + deepest-inspection (a) instance, but multi-interface delivery + inspection-depth are scale/delivery, not new structure (the v159 broadest-coverage / v164 stack-≠-structure discipline). (a) FAIL (`nahime0`/`illegalstudio` bare handle/org, no disclosed identity → no handle-inferred cultural-peer, v139/v160/v164 discipline). OBSERVES, not control → **NOT LV-C7 #22** (prune/compact act on transcript *files*, not the agent's provider/account/config — the observe-vs-control line; also Wails/Go not Tauri); NOT #18 #8 (no provider aggregation); neither pet (b) nor interrupt-router (c) → NOT a new sub-flavor. **It DID mint 1 corpus-first Library-vocab §C standalone** — "Agent Session-Transcript Maintenance / Housekeeping (prune/compact/search stored transcripts)" — recorded at the Library-vocab layer (`06` §C/§F), NOT here; the v158 ai-token-monitor shape (clean observability instance + 1 corpus-first capability). NOT a #52 claim (165★, age unverified §37.4). §35 STAYS CLEAR (window {v163 GA, v164 GA, v165 GA} = 0 OG; 13 consecutive GA v153→v165). Streak GA:27 → **GA:28·OG:11 [7 ov]** (28th GOAL-ALIGNED PASS). ⚠️ 13th consecutive "tool for operating/monitoring AI coding agents" ship, ZERO piloted.

**v166 ai-agent-session-center post-audit ship (2026-06-15):** clean instance-strengthening of sub-flavor **(b) → N=5 / sub-archetype overall → N=12** (`coding-by-feng/ai-agent-session-center`, a self-hosted **web dashboard** that renders each observed session as a run-state-driven animated **3D robot** [Three.js; 6 models × 8 states driven by Idle/Prompting/Working/Waiting/Approval/Input/Ended] + live terminals + prompt history + tool logs + a prompt queue; Node/Express + React 19 + node-pty + SQLite-WAL; 400+ Vitest + Playwright E2E; MIT; ≈72★/200c/v2.10.30). **0 mints** — the v159 CodexBar / v164 oc-claw shape: its distinctive features are **delivery + blend, not structure** — the **first web-delivered, 3D-robot, dashboard-hybrid (b) instance** (vs the desktop-pet delivery of v154/v155/v156/v164), blending (b) ambient/affective [robots = run-state avatars, the signature] with (c)-approval features [approve/deny tool requests, Approval/Input states, Alert — a minor feature, NOT the core] + a light prompt-queue. **(b)/dashboard-hybrid note:** it co-hosts dashboard panels (terminals/logs/queue) alongside the affective robots, softening sub-flavor (b)'s "not a dashboard" parenthetical — counted in (b) because the run-state-driven robot avatars are the primary signature; flagged for the audit to decide if (b) needs a "web-dashboard-hybrid" sub-note. (a) FAIL (`coding-by-feng` bare handle; "feng" = a Chinese-surname *heritage hint* only, no disclosed identity → no cultural-peer inferred, the v139/v160/v164/v165 discipline + v151-audit rec (ii) "require a *solid* (a) signal, not a name/heritage inference"). **OBSERVES, does not host** (README: *"the hooks are purely observational"*; events → `/tmp/.../queue.jsonl` → WebSocket; users launch the CLIs independently; node-pty terminals attach/display) → **NOT the cmux/AoE session-multiplexer species** (resolves the intake wrinkle; that species stays N=2). OBSERVES + approve/deny *pass-through of the agents' own prompts* + kill/archive/delete *sessions* ≠ provider/account/config control → **NOT LV-C7 #22**; no provider routing → **NOT #18 #8**; a web-dashboard blend of existing sub-flavors → **NOT a new sub-flavor** (a 4th "dashboard/mission-control" sub-flavor would inflate — the v160/v165 discipline). Observation (NOT minted): prompt-queue + auto-send-when-idle = **orchestration-lite WATCH** (a 2nd "observe + queue/dispatch prompts to idle sessions" subject could justify a sub-flavor — the v156 permission-bubble precedent); hooks-event-file `/tmp/...queue.jsonl` mechanism = cousin of the JSONL-parsing (a) instances (v89/v158) but event-emitting hooks, NOT reading the agents' own session JSONL; #84 cross-tool (3 agents + OpenCode/Cursor/Windsurf compat). ⚠️ cataloguing-hazard (v148 lesson): `asheshgoplani/agent-deck` (a different TUI session manager) is NOT this subject; canonical = `coding-by-feng/ai-agent-session-center` per the operator URL. **0 new §C standalones; CONFIRMED Library-vocab UNCHANGED at 10; top-level count UNCHANGED at 46.** NOT a #52 claim (≈72★, creation date not shown → velocity unestablished §37.4). §35 STAYS CLEAR (window {v164 GA, v165 GA, v166 GA} = 0 OG; 14 consecutive GA v153→v166). Streak GA:28 → **GA:29·OG:11 [7 ov]** (29th GOAL-ALIGNED PASS). ⚠️ 14th consecutive "tool for operating/monitoring AI coding agents" ship, ZERO piloted — built at explicit operator election after being flagged; pilot one or run the overdue audit, not a v167 in this niche.

**v167 mini-audit (2026-06-16) — observability calls VERIFIED + reconciled:** sub-archetype **N=12 VERIFIED** = (a) metering 6 [v89/v109/v157/v158/v159/v165] + (b) ambient/affective 5 [v154/v155/v156/v164/v166] + (c) attention-routing 1 [v160]; arithmetic clean (6+5+1=12). **(b) "web-dashboard-hybrid" sub-note formalized** (v166; NOT a split / NOT a new sub-flavor). **(c) stays N=1** (only v160; promotion-eligible at N=2). No double-count (the v158 desktop-pet→(b) reconcile stands). **Adjacent N=2 buckets HOLD (no clean 3rd in v164–v166): session-hosting-multiplexer species (cmux v99 + AoE v162) HOLD N=2; Multi-Vendor Orchestration-Platform standalone (Paseo v150 + ai-maestro v163) HOLD N=2 — both PROMOTION-ELIGIBLE at N=3; anti-inflation: don't promote without a clean 3rd** (v164/v165/v166 are all observability; v166 verified OBSERVES-not-hosts). Counts UNCHANGED (46 patterns / 10 Library-vocab CONFIRMED); streak UNCHANGED (audit ≠ ship) GA:29·OG:11 [7 ov]; §35 CLEAR (14 consecutive GA v153→v166). Override-review DISCHARGED 7th time (intake-channel common-cause; NO 4th governor). NO §28 retires (June-burst N=1 standalones all <30 days as of 2026-06-16). ⚠️ INFRA finding (CORRECTED 2026-06-16 post-test): the v167 fan-out workflow died with all 20 subagents over the 200K context limit, BUT a post-compaction subagent smoke-test STILL failed at ~213.9K (was ~213.8K, ≈unchanged) — so the cause is the TOOL-DEFINITION CATALOG + system prompt (connected-MCP/deferred-tool set), NOT the shim. CLAUDE.md was compacted 640KB→29KB as main-loop hygiene (good, but NOT the subagent fix); the workflow/subagent block is a config issue (trim MCP tools and/or re-test in a fresh session) — STILL OPEN. Fact-verification ADVANCED (8 live WebFetch spot-checks, 0 hard corrections; 1 minor v161 framing-refinement). Full doc: `04 Reviews/(C) 2026-06-16 … v167 …`. ⚠️ 14 consecutive in-niche ships, ZERO piloted — pilot / vary-domain / do the shim-rebuild, not a v167-in-niche.

**v160 ping-island post-audit ship (2026-06-05):** mints **NEW sub-flavor (c)** above → **sub-archetype N=8 → N=9** (`erha19/ping-island`, native-macOS Swift notch/Dynamic-Island attention-router for 13 agents via managed hooks + codex app-server WS + SSH bridge). **0 new §C standalones; 0 new CONFIRMED Library-vocab (stays 10); top-level count UNCHANGED at 46.** (a) FAIL (`erha19` no disclosed identity → no handle-inferred cultural-peer, v139 / v151-(a)-rescue discipline). Genuine mint (contrast v159 CodexBar = clean metering instance, 0 mints). Observe+route, NOT control → NOT LV-C7 #22.

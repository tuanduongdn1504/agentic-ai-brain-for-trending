> ⚠️ **HEADER FROZEN at a v78 snapshot (flagged v182 audit, 2026-06-22).** This running header stopped being maintained per-ship after v78; per-ship narrative moved to `_state/03c-projects-v61-v181.md` + `_patterns/05-recent-additions.md`, and Library-vocab observations now file into `_patterns/06-library-vocab-registry.md`. The numbered candidate definitions BELOW remain valid; the "29 active / ratio 0.644" figures in this header are a v78 snapshot, NOT live. **Live state lives in the audit docs (`04 Reviews/`) + the registry.** At v182: active ≈29, confirmed 46 → ratio ≈0.63 (neither count nor ratio trigger firing). Full refresh-or-supersede of this file deferred to a housekeeping pass.

## Active Candidates (29 — post-v78 wiki affaan-m/ECC corpus-recursive revisit 2026-05-20: 0 NEW top-level patterns + **Pattern #52 NEW sub-class "Multi-Month-Sustained EXTREME-VIRAL" (>300/d × 90+d) N=1 PROMOTION-LOCKED-IN** at v78 — v78 ECC at 1,535 stars/day × 122 days verified GitHub API 2026-05-20 = CORPUS-FIRST instance of sustained-multi-month-EXTREME-VIRAL velocity profile; distinct from existing EXTREME-VIRAL sub-class (re-labeled as "pulse" with v63 Karpathy single-artifact + v68 zero 2-day-old anchors); sub-class taxonomy expands 4-variant → 5-variant at v80 audit formal registration; CORPUS-RECORD 1,535/d velocity exceeds prior corpus records (v63 ~1,186/d + v68 1,050/d); retroactive v1 anchor evidence (corpus-recursive validation supports N=1 PROMOTION-LOCKED-IN within already-CONFIRMED Pattern); **Pattern #84 84c.1 N+1 strengthening at repo-stored layer** (ECC ships 11+ harness directories — .claude/.codex/.cursor/.opencode/.gemini/.kiro/.trae/.qwen/.zed/.codebuddy/.agents — joining v75 14-harness + v76 15+-harness sub-sub-mechanism cluster; **7-consecutive-wiki Pattern #84 84c sub-mechanism strengthening arc** v71→v72→v73→v75→v76→v77→v78 = NEW CORPUS-RECORD extending v77's 6-wiki record; sub-sub-mechanism distribution: 84c.1 N=5 + 84c.2 N=2 + 84c.3 N=1); **Pattern #57 STRONGEST corpus-recursive citation at maximum depth + NEW sub-variant 57f candidate "Corpus-Recursive-Anchor-Self-Reference"** PROVISIONAL N=1 (v1 → v78 = CORPUS-FIRST corpus-recursive revisitation of an anchor subject at 33-day delta = corpus-self-reference at maximum-possible depth); **CORPUS-RECORD 7+ corpus-subject citations in single-subject product surface** (Claude Code v65 + Codex v62 + Cursor v75/v76 + OpenCode v67 + Gemini v76 + MCP v66/v70/v76 + Anthropic + Trae v75/v77 + Qwen v73 + Kiro v75/v76 + Zed corpus-new = 11+ citations exceeds prior 6-citation record at v77 by 5+); **NEW Library-vocabulary item #13 candidate "OSS-with-hosted-Pro-SaaS-tier-on-MIT-base"** PROVISIONAL N=1 (ECC dual-tier economic model: MIT-OSS free forever + npm packages free + GitHub App freemium + ECC Pro $19/seat/mo + GitHub Sponsors $5+/mo; "OSS stays free forever; Pro funds the maintainer" framing is structurally distinct from prior corpus subjects pure-OSS + pure-commercial + foundation-funded + tier-based-licensing); **NEW Pattern #19 sub-mechanism 19i candidate "Solo-Architect-Community-Augmented-Paid-Pro-Tier"** PROVISIONAL N=1 (Affaan Mustafa single-architect + 170+ community contributors + paid-Pro-tier + Anthropic-Hackathon-Winner + weekly-cadence-at-scale; distinct from 19a/b/c/d/e/f/g/h); **Pattern #78 STRONGEST single-subject LDS-tracking evidence** with 6-layer LDS surface (manifests/ + schemas/ + research/ + contexts/ + 232-skill codification with SKILL.md template + 60-agent codification with frontmatter conventions + cross-harness architecture doc + Hermes operator release notes; CORPUS-RECORD codification density; NEW sub-mechanism 78d candidate "Operator-System-LDS-with-6-layer-codification"); **Pattern #82 STRONGEST single-subject quantitative-marketing density** (9+ quantitative claims in README first 25 lines: 182K+ stars / 28K+ forks / 170+ contributors / 12+ language ecosystems / Anthropic Hackathon Winner / 150 GitHub App installs / 7-harness multi-target / 232 skills / 60 agents / 75 commands / 10+ month daily-use); **Pattern #66 CORPUS-FIRST supply-chain-discipline-at-velocity** (ecc-agentshield security-scanning npm package + Prompt Defense Baseline in CLAUDE.md corpus-first explicit prompt-injection-defense at project scope + security-reviewer agent + TDD 80%+ coverage discipline + single-maintainer-weekly-cadence-discipline-at-187K-star-scale); **2 NEW observational candidates**: OC-AC "Anthropic-Hackathon-Winner-Status as corpus-signal-axis" N=1 (corpus-first explicit hackathon-status identity-declaration) + OC-AD "Community-Augmented-Solo-Architect-at-Velocity" N=1 (solo-architect + 170+ contributors + sustained-multi-month-EXTREME-VIRAL combination; corpus-first structural pattern). Ratio 29:45 = 0.644 UNCHANGED (sub-class N=1 PROMOTION-LOCKED-IN + within-pattern + sub-mechanism + 2 NEW OCs only; no top-level state transitions; Pattern #52 sub-class formalization at v80 audit administrative). OC count 34 → **36** (+OC-AC + OC-AD). Library-vocabulary items 12 → **13** (with #13 PROVISIONAL N=1). Storm Bear **STRONGEST INCLUDE 4/4** — (a) **NEW axis-call PASS at VN-cultural-locale-inclusive-product scope** (ECC ships `docs/vi-VN/README.md` first-class i18n = CORPUS-FIRST locale-inclusion-as-(a)-evidence axis-call distinct from prior v76 direct-author-peer + v77 community-org PROBABLE PASS + v73 Chinese-handle-inferred) / (b) STRONGEST PASS direct vault deployment as Claude Code skill collection + multi-harness applicability + reversible + already-corpus-evaluated as deployable at v1 / (c) STRONGEST PASS v1 anchor + corpus-recursive subject + 5+ routine v2.3 codification candidates contributed at single subject / (d) STRONGEST PASS 7+ corpus-subject citations = CORPUS-RECORD citation density. **Streak=14 NEW CORPUS-RECORD extension by 1 wiki** post-v77 13-record. 22-instance Phase 0.9 post-amendment window 90.9% INCLUDE rate (uptick from 90.5%). Strength distribution v65-v78: STRONGEST × 4 + STRONG × 2 + MODERATE × 4 + WEAK × 4 (balanced spectrum across 14-wiki window; STRONGEST × 4 reflects late-window maturation enabling clearer evidence-thresholding). **Library-vocabulary item #9 Cross-Pattern Coupled-Design 9-pattern couple-count** (Pattern #52 + #84 84c.1 + #57 57f + Library-vocab #13 + Pattern #19 19i + Pattern #78 78d + Pattern #82 + Pattern #66 + Pattern #21 legacy-shim) = NEW CORPUS-RECORD couple-count extending v77's 7-pattern record + v76's 8-pattern record. **14 corpus-firsts at v78 wiki** = top single-wiki corpus-first count in routine v2.2 window. Routine v2.2 fourteenth-wiki validation clean + 5 NEW routine v2.3 codification candidates contributed (corpus-recursive anchor revisitation policy + multi-month-sustained-velocity sub-class formalization + product-locale-inclusion as Storm Bear (a) sub-criterion + dual-license-economic-model artifact registration + retroactive-anchor-evidence eligibility for sub-class N=1 PROMOTION). **HIGH-RELEVANCE vault pilot candidate position #3 at fenced-scope** (cc-switch #1 / agent-skills-standard #2 / **ECC NEW v78 #3 fenced-scope** / codegraph #4 / agents-best-practices #5 / agentmemory #6). Active count 29 maintained 7th-consecutive-wiki post-v72-audit relief = CORPUS-RECORD audit-relief sustainability EXTENDED by 1 wiki. Prior post-v77 wiki datawhalechina/easy-vibe 2026-05-20: 0 NEW top-level patterns + **Pattern #51 NEW sub-pole "PRO-VIBE-explicit" N=1 PROVISIONAL anchor at v77** (CORPUS-FIRST explicit-affirmative-PRO-VIBE-named subject — tagline "vibe coding 2026" + secondary "Jump right in and vibe together — if you can talk, you can build apps" + inverted-learning-order build-first-understand-after + conversation-as-product framing + 13.2K stars + 1.3K forks community-validation + Datawhalechina + Tsinghua academic endorsement + 120+ topic curriculum organized entirely around methodology + methodology-naming-as-citable-handle "vibe coding 2026"; v77 fills known Pattern #51 spectrum gap at affirmative-end; PROMOTE-AS-NEW-SUB-POLE HIGH confidence at v80 audit if N=2 sustains OR REGISTER as singular-anchor sub-pole); **Pattern #83 sub-mechanism 83f "license-declaration-vs-actual-mismatch" N=3 PROMOTION-LOCKED-IN** (v74 LLMs-from-scratch CITATION.cff Apache-2.0 vs LICENSE.txt modified Apache + v76 agent-skills-standard README MIT vs LICENSE Apache-2.0 + v77 easy-vibe README CC BY-NC-SA 4.0 vs GitHub API "None specified" = 3-instance arc within 3 consecutive wikis = CORPUS-RECORD sub-mechanism promotion-arc-speed under routine v2.2; 3 mechanism-variants 83f.1 within-LICENSE / 83f.2 across-doc-LICENSE / 83f.3 across-doc-API; v80 audit formal sub-mechanism promotion administrative); **NEW Library-vocabulary item #12 candidate "AI-Optimized Tutorial Index via llms.txt"** PROVISIONAL N=1 (corpus-first artifact-type: 3+1-stage architecture + quick decision tree user-intent→tutorial-section routing + dual-layer index directory + detailed-article + 120+ articles × 20+ keywords each + answer specifications citation-format + code-first strategy + stage-matching + version management + 10+ language versions; distinct from v75 impeccable HARNESSES.md cross-harness spec matrix + v76 agent-skills-standard AGENTS.md 3-tier hierarchical lookup); **NEW Pattern #19 sub-mechanism 19h candidate "Chinese-Educational-Community-Organization-with-University-Anchor"** PROVISIONAL N=1 (Datawhalechina community-org + Sanbu project lead + Fang Ke Tsinghua mentor + 5+ named Tsinghua student contributors; distinct from 19a Western solo-individual + 19b solo-developer multi-project + 19c foundation-organization-only + 19d vendor-organization + 19e educator-multi-book + 19f methodology-influence-node single-individual + 19g Vietnamese-Solo-Standards-Codifier); **Pattern #84 84c N=6 strengthening with NEW sub-sub-mechanism 84c.3 "curriculum-mediated harness exposure"** (5-stage curriculum teaches Claude Code + Cursor + Trae + VS Code AI + Dify as peer-tools across modules; distinct from 84c.1 explicit-provider-matrix v71/v72/v73 + 84c.2 CLI-generates-native-formats v75/v76; CORPUS-RECORD 6-instance Pattern #84 sub-mechanism-strengthening arc post-promotion; 84c sustainability bulletproof + sub-sub-mechanism taxonomy formalization at v80); **Pattern #52 SUSTAINED-MODERATE-HIGH sub-class N=2 evidence** (91.7 stars/day × 144 days; PROMOTION-ELIGIBLE at v80 N=3); **Pattern #78 NEW sub-mechanism 78c candidate "Educational-Curriculum-LDS-with-AI-routing"** PROVISIONAL N=1 (120+ articles + 20+ keywords-per-article + decision tree + dual-layer index + answer-specifications + 10+ language versions; distinct from 78a CLI-enforceable + 78b multi-protocol-tracking + triple-standard); **Pattern #45 NEW sub-typology 45e candidate "Non-Commercial-Educational-CC-License"** (CC BY-NC-SA 4.0 educational content NC-licensing; distinct from 45a-d Apache/MIT mechanisms) OR Pattern #29 absent-LICENSE strengthening (v80 audit decides framing); **Pattern #57 sub-variant 57c-product strengthening** (6+ corpus-subject citations Claude Code + MCP + Cursor + Trae + VS Code AI + Dify in single subject — STRONG 57c evidence); **2 NEW observational candidates**: OC-AA "Inverted-Learning-Order-Pedagogy" N=1 (build-first + multi-tool-dialogue-fluency + conversation-as-product + conversational-onboarding 4-axis inversion) + OC-AB "Multi-Deploy-Intelligence-via-Base-Path-Auto-Detection" N=1 (5-deploy-target Vercel + GitHub Pages + Azure + Docker + nginx with runtime base-path detection). Ratio 29:45 = 0.644 UNCHANGED (within-pattern + sub-pole + sub-mechanism + sub-sub-mechanism + 2 NEW OCs only; no top-level state transitions). OC count 32 → **34** (+OC-AA + OC-AB). Storm Bear **STRONG INCLUDE 3-4/4** — criterion (a) PROBABLE PASS at Asian-cultural-peer community-education scope (CORPUS-FIRST inference-tolerant criterion-(a) call for community-org subject; Datawhalechina + Tsinghua University Mainland-China-located community-org + academic-anchor); (b) PASS MODERATE-RELEVANCE vault pilot pedagogy cross-reference; (c) STRONG PASS methodology-influence-node Pattern #51 spectrum-anchor + 5 routine v2.3 codification candidates; (d) STRONG PASS 6+ corpus-subject citations. Streak=13 NEW CORPUS-RECORD extension by 1 wiki post-v76 (v76 was prior 12-record). 21-instance Phase 0.9 post-amendment window 90.5% INCLUDE rate (uptick from 90.0%). Strength categorization v65-v77: STRONGEST × 3 + STRONG × 2 + MODERATE × 4 + WEAK × 4 (broadly balanced spectrum across 13-wiki window). **Library-vocabulary item #9 Cross-Pattern Coupled-Design 7-pattern couple-count** (Pattern #51 + #83 83f + Library-vocab #12 + Pattern #19 19h + Pattern #84 84c + Pattern #52 + Pattern #78 78c). 11 corpus-firsts at v77 wiki. Routine v2.2 thirteenth-wiki validation clean + 5 NEW routine v2.3 codification candidates contributed (curriculum-mediated harness exposure as Pattern #84 sub-sub-mechanism + PRO-VIBE-explicit as Pattern #51 spectrum-anchor sub-pole + sub-sub-mechanism taxonomy formalization 5-layer hierarchy + community-org + university-anchor archetype + AI-Agent-optimized tutorial index artifact-type). MODERATE-RELEVANCE vault pilot candidate (pedagogy cross-reference; pilot ranking unchanged — cc-switch #1 + agent-skills-standard #2 + codegraph #3 + agents-best-practices #4 + agentmemory #5). Active count 29 maintained 6th-consecutive-wiki post-v72-audit relief = CORPUS-RECORD audit-relief sustainability EXTENDED by 1 wiki. Prior post-v76 wiki HoangNguyen0403/agent-skills-standard 2026-05-19: 0 NEW top-level patterns + **Pattern #84 84c "Provider-Agnostic-By-Design" N=5 POST-PROMOTION STRENGTHENING with NEW sub-mechanism "CLI-generates-native-formats-from-canonical-source"** (5-instance Pattern #84 sub-mechanism-strengthening arc within post-promotion window = NEW CORPUS-RECORD extending v75 4-instance record; CORPUS-FIRST CLI-generation distribution mechanism distinct from v75 impeccable repo-stored-N-harness-replicas; CORPUS-FIRST sub-mechanism mechanism-layer-distinction within 84c distribution layer with 2 sub-sub-mechanisms 84c.1 repo-stored + 84c.2 CLI-generated; methodology + runtime + distribution layers + 2 sub-sub-mechanisms = 84c fully mature sub-mechanism); **Pattern #78 N=4 CORPUS-RECORD codification density** (259 skills × 20+ frameworks at ~500 tokens/skill avg + 3-tier hierarchical lookup AGENTS.md router → category _INDEX.md → SKILL.md on-demand = O(n) → O(1) scan cost + evals.json per skill CI-validated + benchmark-report.md methodology document + MCP Transparent-Interception with audit logs corpus-first audit-trail enforcement layer); **Pattern #19 NEW sub-mechanism 19g candidate "Vietnamese-Solo-Standards-Codifier"** PROVISIONAL N=1 (Hoang Nguyen 4-product portfolio: agent-skills-standard + sheet_loader_localization + mvvm_architecture + ai_code_reviewer; solo VN engineer + mobile-Flutter foundation + standards-codification anchor; distinct from 19a/b/c/d/e/f); **Pattern #83 sub-mechanism 83f "license-declaration-vs-actual-mismatch" N=2 PROMOTION-ELIGIBLE** (v74 LLMs-from-scratch CITATION.cff "Apache-2.0" vs LICENSE.txt modified Apache + v76 agent-skills-standard README "MIT License" vs LICENSE Apache-2.0 = N=2 promotion threshold reached; v77 audit formal sub-mechanism promotion expected); **Pattern #82 N+1 quantitative-marketing strengthening** ("85% fewer tokens" + 540 tokens/skill avg + 90% scouting-token reduction + benchmark-report.md methodology document + evals.json per skill CI validation); **Pattern #18 sub-mechanism B B1 MCP variant N=3 strengthening + NEW within-B1 sub-variant candidate "MCP-Transparent-Interception-with-audit-logs"** (v66 agentmemory N=1 + v70 codegraph N=2 + v76 agent-skills-standard N=3; "Transparent Interception" intercepts file read requests + auto-injects rules + audit logs); **Pattern #66 6-axis supply-chain integrity discipline** (text-only artifacts + CLI text-only downloads + zero telemetry + local-first + prompt-injection-scanning + Zod schema validation + per-skill evals.json); **Pattern #57 sub-variant 57c-product strengthening** (3+ corpus subjects cited as ecosystem peers: v62 codex-plugin-cc + v65 claude-code-system-prompts + v71 agents-best-practices ecosystem); **3 NEW observational candidates**: OC-X "Audit-Log-As-Enforcement-Layer" N=1 (MCP audit logs showing which skills informed each decision) + OC-Y "Engagement-Deficit-Extreme-With-Active-Forks" N=1 (0 open issues despite 147 forks; active project per 144 commits; community via forks/PRs not issues) + OC-Z "VN-Solo-Developer-Standards-Codifier" N=1 (CORPUS-FIRST VN-located archetype in v60+ window). Ratio 29:45 = 0.644 UNCHANGED (within-pattern strengthening + 3 NEW OCs only; no top-level state transitions). OC count 29 → **32** (+OC-X + OC-Y + OC-Z). Storm Bear **STRONGEST INCLUDE 4/4** — FIRST 4/4 STRICT PASS since v66 agentmemory (10-wiki gap STRONGEST recurrence with FULL criterion-PASS); (a) STRONGEST PASS Nguyen Huy Hoang = Senior Software Engineer at VMO Holding + **Hanoi, Vietnam** = direct structural-peer to Storm Bear; **CORPUS-FIRST VN-located solo developer in v60+ window post-pilot-v3.2 hireui** (45-wiki gap); (b) PASS direct vault deployment via npm CLI + MCP server; (c) PASS methodology-influence-node 4 routine v2.3 codification candidates; (d) PASS multi-harness ecosystem citation 3+ corpus subjects. Streak=12 NEW CORPUS-RECORD extension by 1 wiki; 19-instance window 89.5% INCLUDE rate (uptick from 88.9%). **Library-vocabulary item #9 Cross-Pattern Coupled-Design potential CORPUS-RECORD 8-pattern couple-count** (Pattern #84 + #78 + #19 + #82 + #83 83f + #18 + #66 + #57). **Library-vocabulary item #11 Engagement-Deficit-Extreme corpus-record-low strengthening** (0 open issues / 0 issue-rate despite 147 forks + active 144 commits; highest-fork-with-lowest-issue corpus subject). **CORPUS-RECORD-HIGH fork ratio 0.296** (1.66× v65 17.8% prior record). 10 corpus-firsts at v76 wiki. Routine v2.2 twelfth-wiki validation clean + 4 NEW routine v2.3 codification candidates contributed (mechanism-layer-distinction within sub-mechanism layer + MCP Transparent-Interception with audit-log enforcement + 3-tier hierarchical lookup as token-efficient architecture pattern + CLI-generates-native-formats discipline). **HIGH-RELEVANCE vault pilot candidate Tier-1** ranks #2 (cc-switch #1 / agent-skills-standard #2 NEW v76 / codegraph #3 / agents-best-practices #4 / agentmemory #5) — VN-cultural-peer methodology + standards-codification + zero-infra + multi-relevance dimensions. Active count 29 maintained 5th-consecutive-wiki post-v72-audit relief = CORPUS-RECORD audit-relief sustainability EXTENDED by 1 wiki. Prior post-v75 wiki pbakaus/impeccable 2026-05-19: 0 NEW top-level patterns + **Pattern #84 84c "Provider-Agnostic-By-Design" N=4 POST-PROMOTION STRENGTHENING with NEW CORPUS-RECORD 14-harness multi-target distribution** (4-instance monotonic corpus-record migration across 4 consecutive wikis v71 dual-vendor → v72 9-provider → v73 12-provider → v75 14-harness; CORPUS-FIRST 4-instance Pattern #84 sub-mechanism-strengthening arc post-promotion; CORPUS-FIRST monotonic-corpus-record migration across 4 consecutive wikis; CORPUS-FIRST sub-mechanism layer change-of-measurement-axis = methodology layer v71 → runtime layer v72-v73 → distribution layer v75; 84c sustainability bulletproof; v76 stale-check + v80 retire-check RELIEVED); **Pattern #57 STRONG corpus-recursive citation** (2-source NOTICE.md derivative-attribution chain: extends Anthropic frontend-design skill Apache 2.0 + incorporates ehmo/typecraft-guide-skill tactical additions; HIGH but not MAXIMUM depth; NEW sub-variant candidate **57e "Multi-Source-Derivative-Attribution-Chain via NOTICE.md"** PROVISIONAL N=1); **Pattern #52 HIGH-NOT-EXTREME sub-class N=4 lower-edge confirmation** (impeccable 155.8/d × 185d completes velocity-band edge coverage 155-268/d + age-span coverage 86-289d; v75 audit Pattern #52 top-level PROMOTE administrative-only); **Pattern #19 N+1 + NEW sub-mechanism 19f candidate "Creative-Technologist-Multi-Product-Portfolio"** (Bakaus jQuery UI + Google for Creators + Spotter Studio + impeccable + radiant + agent-reviews 5+-domain breadth-focus portfolio at corporate-employer scale; distinct from 19a/b/c/d/e depth-focus sub-mechanisms); **Pattern #78 78a CLI-enforceable-LDS strengthening** (27 anti-pattern deterministic rules + `npx impeccable detect` standalone scanner = corpus-first enforceable-LDS layer; stronger than v71 78a triple-standard); **Pattern #28 sister-mechanism candidate "Editorial-Build-Validator"** PROVISIONAL N=1 (STYLE.md denylist CI-enforced; distinct from v73 anti-bloat-via-documented-refusal); **Pattern #18 NEW sub-mechanism candidate "one-skill-many-HARNESS-CONSUMERS"** PROVISIONAL N=1 (consumer-side inverse of v70 promoted sub-mechanism B "one-binary-many-CLIENTS"; impeccable single-skill replicated into 14 harness-target directories); **3 NEW observational candidates**: OC-U "Editorial-Voice-Denylist-As-Build-Validator" N=1 + OC-V "Multi-Source-Derivative-NOTICE-Attribution" N=1 + OC-W "Anti-Pattern-Enumeration-With-CLI-Detector" N=1. Ratio 29:45 = 0.644 UNCHANGED (within-pattern strengthening + 3 NEW OCs only). OC count 26 → **29** (+OC-U + OC-V + OC-W). Storm Bear **STRONG INCLUDE 3/4** (a FAIL Bakaus = Google Creative Technologist not Storm Bear structural-peer; b PASS direct vault skill deployment; c PASS methodology-influence-node sub-foundation scale; d PASS corpus-recursive HIGH-depth via Anthropic frontend-design parent-skill citation + ehmo tactical-additions); streak=11 CORPUS-RECORD extension; STRONG returns after 9-wiki gap (last STRONG was v66 agentmemory); 18-instance window 88.9% INCLUDE rate UNCHANGED. **Library-vocabulary item #9 Cross-Pattern Coupled-Design potential CORPUS-RECORD 6-pattern couple-count** (Pattern #84 84c + Pattern #57 57e + Pattern #19 19f + Pattern #78 78a + Pattern #28 sister + Pattern #18 NEW sub-mechanism). 8 corpus-firsts at v75 wiki. Routine v2.2 eleventh-wiki validation clean + 4 NEW routine v2.3 codification candidates contributed (editorial-discipline-as-build-validator + multi-source-derivative-attribution-chain-via-NOTICE.md + harness-installation-pathway-as-distinct-distribution-axis + anti-pattern-enumeration-with-deterministic-detector). NOT a vault pilot candidate (no active UI work; pilot ranking unchanged from post-v74). Active count 29 maintained 4th-consecutive-wiki post-v72-audit relief = corpus-record audit-relief sustainability. Prior post-v74 LLMs-from-scratch wiki 2026-05-19: 0 NEW top-level patterns + **NEW Pattern #45 sub-typology 45d "Apache-2.0 with Book-Content-Exclusion-from-Source"** PROVISIONAL N=1 (SECOND formal sub-typology proposal-document in corpus history after v69's 45c; corpus-first intra-Apache modification carving book-content + images out of "Source form" definition; HIGH confidence verdict; v75 stale-check + v80 retire-check; formal sub-variant promotion at N=2 emergence — retroactive `rasbt/machine-learning-book` Packt 2022 companion check could provide retroactive N=2); **NEW T3 sub-archetype "Educational-Book-Companion"** corpus-first in v60+ window (book-as-primary-artifact + chapter-folder-mirrors-book-TOC + print-book-constraint-shapes-architecture); **NEW Pattern #19 sub-mechanism 19e "educator-multi-book-portfolio"** candidate N=1 (Raschka 6 pinned repos: LLMs-from-scratch + reasoning-from-scratch + machine-learning-book + deeplearning-models + llm-architecture-gallery + mini-coding-agent; distinct from sub-mechanisms 19a/b/c/d); **NEW Pattern #52 sub-class "LONG-SUSTAINED-MEDIUM-velocity (60-150/d × 1000+d)"** candidate N=1 (92.2 stars/day × 1,032 days = corpus-first multi-year-sustenance subject; distinct from EXTREME-VIRAL >300/d + HIGH-NOT-EXTREME 150-300/d + SUSTAINED-MODERATE-HIGH 25-150/d×<1000d; v75+ stale-check); 3 NEW observational candidates: **OC-R "Affiliate-Link-As-Repository-Homepage"** N=1 (homepage `amzn.to/4fqvn0D` Amazon affiliate redirect; corpus-first homepage-as-affiliate-link) + **OC-S "Methodology-Influence-Node-Without-Operational-Tool" N=2 EARLY-PROMOTION-ELIGIBLE** (v63 Karpathy + v74 Raschka — Phase 0.9 (c) STRONG PASS without (b) PASS; criterion 4 variant-within-pattern-at-N=2 promotion candidate at v75+ audit) + **OC-T "Print-Book-Constraint-As-Architecture"** N=1 (5 manifestations: main-chapter-content-locked + bonus-folders-open + LDS-evolution-constrained-to-bonus + license-carve-out-for-print + git-clone-only-distribution); Pattern #83 sub-mechanism 83f sister-candidate "license-declaration-vs-actual-mismatch" (CITATION.cff "Apache-2.0" declaration vs LICENSE.txt modified Apache reality; N=1 sister observation; v75+ N=2 emergence check); Pattern #78 N+1 LDS strengthening (book-content-stability + bonus-folder-LDS-evolution dual-layer with Llama/Qwen/Gemma model architecture supplements); Pattern #19 N+1 top-level strengthening (Raschka 149 public repos + 6 pinned + ~124K combined pinned-stars); Library-vocabulary item #9 Cross-Pattern Coupled-Design strengthening (4-pattern correlation at single wiki + NEW "archetype-derived-pattern-cluster" sub-mechanism candidate where one archetype constraint shapes multiple patterns simultaneously). Ratio 29:45 = 0.644 UNCHANGED (within-pattern strengthening + sub-typology + sub-archetype + sub-mechanism candidates only; no top-level state transitions). Active count 29 maintained 1-wiki-below-trigger-threshold (3rd consecutive wiki at 29 = audit-relief-sustained). OC count 24 → **26** (+OC-R + OC-T register at N=1; OC-S registers at N=2). Storm Bear meta-streak advances to 10 post-v64-RESET (Phase 0.9 STRICT 1/4 PASS = WEAK INCLUDE; (c) STRONG via Raschka methodology-influence; returns to WEAK after v73 STRONGEST — spectrum re-expanded across STRONGEST→WEAK in 2 consecutive wikis); 18-instance window 88.9% INCLUDE rate. Prior post-v73 wiki farion1231/cc-switch 2026-05-19: 0 NEW top-level patterns + **Pattern #52 HIGH-NOT-EXTREME sub-class N=3 PROMOTION-LOCKED-IN with CORPUS-RECORD 289-day sustenance** (cc-switch 195/day at 289-day repo age — sustained mid-life-cycle case post-launch-spike validates HIGH-NOT-EXTREME 150-300/d band as sustainable not just launch-window phenomenon; CORPUS-FIRST 1-wiki gap from N=2 v72 → N=3 v73 = sustained-velocity sub-class promotion via criterion 4 variant-within-pattern at v75 audit LOCKED-IN HIGH confidence); Pattern #84 84c THIRD POST-PROMOTION strengthening (CORPUS-RECORD 12-provider matrix supersedes v72 9-provider — corpus-record migration in 1 wiki); Pattern #57 STRONGEST evidence post-v64-RESET (4 LLM substrate vendors cross-referenced single subject: Claude Code + Codex CLI + Gemini CLI + Qwen CLI — corpus-record cross-vendor density); Pattern #28 PROVISIONAL "Anti-Bloat-Discipline-Documented-Refusal" candidate (README explicit refusal of feature creep with stated rationale); Pattern #66 sub-mechanism 66f PROVISIONAL "Local-config-snapshot-as-rollback-strategy"; Pattern #78 within-pattern OSC-7 + OSC-9 + OSC-133 + LSP-3.18 multi-protocol-tracking strengthening; Pattern #21 within-pattern session-handoff-via-README-checkpoint sub-variant; Pattern #18 sub-archetype #8 PROVISIONAL "multi-runtime-aggregator" (cc-switch aggregates 5+ Claude-Code runtime variants under single config-management UI — DISTINCT from sub-mechanism C "dispatcher-companion-binary-split" registered v72); **NEW observational candidate OC-Q "Sponsor-Density-As-Corpus-Signal"** (cc-switch lists 4+ named Chinese sponsors in README — first corpus subject with explicit sponsor-density display as community-validation signal). OC count 23 → **24** with OC-Q. Ratio 29:45 = 0.644 UNCHANGED (within-pattern strengthening + 1 NEW OC only). Active count 29 maintained 1-wiki-below-trigger-threshold post-v72-audit relief (2nd consecutive wiki at 29 = audit-relief-sustained). Prior post-v72 wiki Hmbown/DeepSeek-TUI 2026-05-19: 0 NEW top-level patterns + Pattern #52 **HIGH-NOT-EXTREME sub-class N=2 PROMOTION-ELIGIBLE** (CORPUS-FIRST 0-wiki-gap audit-policy-satisfaction; v72 audit registered sub-class at morning; v72 wiki provides N=2 evidence at afternoon; v75 audit Pattern #52 top-level promotion path eligible via criterion 4 variant-within-pattern); Pattern #84 84c FIRST POST-PROMOTION strengthening (9-provider matrix N=2 within sub-mechanism; fastest sub-mechanism N=2 in corpus history; RELIEVES v76 retire-check threat); Pattern #21 sub-variant candidate "Session-Longevity-Discipline-As-Pattern" (AGENTS.md 6-rule discipline); Pattern #66 sub-mechanism 66e PROVISIONAL "GitHub-issue-as-attack-surface"; Pattern #18 sub-mechanism C PROVISIONAL "dispatcher-companion-binary-split"; Pattern #78 within-pattern 6-layer LDS strengthening; Pattern #83 83b + 83d multi-surface; **NEW observational candidate OC-P "Community-Built-Canonical-Vendor-Client"** (solo dev builds canonical client for vendor API at 32K-star scale + 9-provider matrix portability). OC count 22 → **23** with OC-P. Ratio 29:45 = 0.644 UNCHANGED (within-pattern strengthening + 1 NEW OC only). Active count 29 maintained 1-wiki-below-trigger-threshold post-v72-audit relief. Prior post-v72 PROMOTION-DRIVEN + 5TH-CONSECUTIVE-TRIGGER mini-audit 2026-05-19: **Pattern #84 Cross-Vendor Ecosystem-Tolerance PROMOTED to CONFIRMED** under criterion 1 + criterion 4 with N=3 evidence v62+v65+v71 (sub-taxonomy 84a Tool-tolerance / 84b Usage-enforcement / 84c Provider-Agnostic-By-Design registered); **Pattern #18 sub-archetype shared-backend-service sub-mechanism B PROMOTED** to formal sub-archetype with protocol-variants B1 MCP N=2 + B2 CDP N=1 + B3 placeholder; **Pattern #66 sub-mechanism 66d "Malicious-skill-package supply-chain layer" PROVISIONAL registration**; **Pattern #52 3-tier velocity sub-classification registered** (EXTREME-VIRAL >300/d + HIGH-NOT-EXTREME 150-300/d + SUSTAINED-MODERATE-HIGH 25-150/d; top-level promotion DEFERRED to v75); OC-E count updated 1 → 2 (v68 + v70 sister observation); E1/E2 2-mode taxonomy DEFERRED to v75+; OC-K/L/N/O confirmed STALE-CANDIDATE; OC-M remains N=2 (no N=3 at v71); Pattern #45 45c first-cycle stale confirmed at N=1; Tier-Taxonomy Review T6 2nd deferral with v75 hard-deadline; Pattern #85 v76 re-evaluation maintained. Ratio 30:44 = 0.682 → **29:45 = 0.644** (active count decrease + confirmed count increase; trigger-state RELIEVED for first time in 5 wikis). Audit document: `04 Reviews/(C) 2026-05-19 Pattern Library mini-audit post-v70-v71 (PROMOTION-DRIVEN + 5TH-CONSECUTIVE-TRIGGER; Pattern #84 + Pattern #18 sub-mechanism B PROMOTIONS + 22 other items).md`. Active-candidates header before v72 audit (preserved for narrative continuity below): v70 codegraph wiki 2026-05-19: 0 NEW top-level patterns + Pattern #18 sub-archetype shared-backend-service **sub-mechanism B N=3 strengthening with 2 protocol-variants formalization** (B1 MCP variant N=2 = agentmemory v66 + codegraph v70 + B2 CDP variant N=1 = CloakBrowser v69; FIRST 4-layer Pattern hierarchy formalized in corpus; FIRST sub-mechanism formalization proposal-document = NEW 6th Phase 4b vehicle); 4 NEW observational candidates (**OC-K Pre-Indexed-Context-Layer** as agent augmentation strategy N=1 + **OC-L Multi-Agent-Installer-Pattern** N=1 (single tool, multiple agent targets via `--target` flag) + **OC-M Quantitative-Benchmark-Marketing** N=2 (v69 CloakBrowser + v70 codegraph both lead with quantitative claims) + **OC-N Auto-generated AGENTS.md from tool installer** N=1); **OC-E Synchronicity-Discipline-As-Policy** sister observation (codegraph 3-file MCP-tool-behavior config-synchronicity vs v68 zero examples+docs+tests+contracts behavioral-synchronicity — mechanism-distinction defer to v71+ audit; 2-mode taxonomy candidate OC-E1 behavioral + OC-E2 configuration); Pattern #19 N+1 founder-personal-multi-product-portfolio strengthening (Colby McHenry 20-repo portfolio + 6 distinct pinned products); Pattern #57 strengthening (2 corpus subjects Codex v62 + OpenCode v67 + Claude Code substrate cited in single subject — STRONGEST (d) of post-v64-RESET window); Pattern #66 strengthening (100% local supply-chain-isolation claim + tree-sitter upstream attribution); Pattern #52 SUSTAINED-MODERATE-HIGH-velocity data point (42 stars/day at 121 days — below EXTREME-VIRAL >300/day threshold but sustained-mid-life-cycle case for v70+ audit batch). Ratio 30:44 = 0.682:1 (UNCHANGED — within-pattern strengthening only; no top-level state transitions). Active count 30 = AT trigger threshold maintained 4th-consecutive-wiki — audit-velocity-control test case. Prior post-v69 OVERDUE-NATURAL-CADENCE mini-audit 2026-05-19: **Pattern #83 Honest-Deficiency-Disclosure PROMOTED to CONFIRMED** under criterion 1 (≥3-cross-tier, N=5 evidence v64+v65+v67+v68+v69 with HIGH confidence; sub-taxonomy 83a/83b/83c/83d/83e registered; full confirmed entry at `_patterns/02b-confirmed-patterns-v42-plus.md`); **Pattern #45 sub-typology 45c absorbed into Pattern #45 confirmed entry** (registration confirmed at audit; v70 stale-check + v75 retire-check; sub-variant promotion at N=2); 4 within-pattern sub-mechanism/sub-variant registrations (Pattern #84 84a/84b + Pattern #81 81a/81b + Pattern #18 shared-backend-service sub-archetype). Pre-v69-audit: 32 active. Audit document: `04 Reviews/(C) 2026-05-19 Pattern Library mini-audit post-v66-v69 (OVERDUE-NATURAL-CADENCE; LARGEST batch corpus-history; Pattern #83 N=5 PROMOTION + 23 other items).md`. Active-candidates header before audit (preserved for narrative continuity below): v69 CloakBrowser wiki 2026-05-18 → 2026-05-19: +1 NEW Pattern #45 sub-typology candidate **45c "Artifact-Scope-Split with Acceptable-Use-Enumeration"** N=1 (corpus-first wrapper-OSS + binary-proprietary-with-acceptable-use license shape; 4 distinguishing mechanisms: two-file artifact-scope split + Acceptable Use enumeration + bounded $100 liability cap + OEM/SaaS B2B carve-out; HIGH confidence sub-typology registration verdict; v70 stale-check + v75 retire-check) + 4 NEW observational candidates (OC-G Fork-as-Integration-Strategy + OC-H Anonymous-Corporate-Author for Dual-Use Tools + OC-I Detection-Evasion-As-Product-Axis + OC-J Acceptable-Use Enumeration as Risk-Mitigation Layer) + Pattern #83 N=5 consolidation evidence (5-disclosure-surface saturation: LICENSE + CHANGELOG + Liability + Acceptable Use + Platform-disparity) + Pattern #52 sub-threshold-control case (CloakBrowser 172/day HIGH-NOT-EXTREME; clarifies >300 threshold discriminates) + Pattern #66 strengthening (3-tier signature verification GPG+attestation+cosign corpus-broadest + 3-level upstream attribution) + Pattern #57 57c-product strengthening (browser-use v34 + crawl4ai v29 named in single README) + Pattern #19 sister observation (corporate-anonymous-product-suite distinct from prior 4 individual-portfolio sub-archetypes); ratio 32:43 = 0.744:1. Active count ≥30 trigger maintained 3rd-consecutive-wiki at v69 — audit-before-v70 recommendation ELEVATED to blocking-priority. Prior v68 zero wiki 2026-05-18: +1 NEW Pattern candidate **T1 sub-archetype #7 "programming-language-as-agent-substrate"** N=1 stale-risk-flagged (corpus-first programming-language subject; PROVISIONAL T1 NEW sub-archetype with v69 Tier-Taxonomy Review T6 audit-decision flag for NEW Tier T6 vs T1 sub-archetype retention) + 7 NEW observational candidates (Skill-As-Binary-Bootstrap + OC-A Agent-First-Language-Substrate + OC-B Self-Hosted Compiler Bootstrap + OC-C In-Browser-Compiler-Via-WASM + OC-D Anti-Compatibility-As-Project-Policy + OC-E Synchronicity-Discipline-As-Policy + OC-F Structured-JSON-as-First-Class-CLI-Output) + Pattern #83 N=4 strengthening (consolidates v67 N=3 PROMOTION-ELIGIBLE verdict) + Pattern #52 EXTREME-VIRAL v68 zero added to v69 sustained-velocity-test batch (1,050 stars/day at 2-day-old = corpus 2nd-highest after karpathy-skills v63 ~1,186/day) + Pattern #81 partial N=2 evidence (5-file version-bump discipline documented but MANUAL not CI-enforced — discipline-documented-but-not-enforced sub-variant) + Pattern #82 WEAK N=2 (per-release contributor lists; different mechanism than v64 Pro Hub Challenge); ratio 31:43 = 0.721:1. Prior v67 opencode-antigravity-auth wiki 2026-05-18: +3 NEW observational candidates (OC-1 Adversarial-Detection-Tolerant Architecture N=1 + OC-2 Issue-As-Enforcement-Signal Pipeline N=1 + OC-3 Quota-Arbitrage-Across-Vendor-Internal-Surfaces N=1) + Pattern #83 advanced to N=3 PROMOTION-ELIGIBLE + Pattern #84 REFINEMENT evidence + NEW Library-vocabulary item #11 Engagement-Deficit-Extreme. v66 agentmemory wiki + v66 EARLY-OPERATOR-ELECTED mini-audit 2026-05-14: -1 Pattern #78 PROMOTED + 5 NEW top-level candidates #80-#84 + Pattern #19 sub-archetype 19a/b/c/d + Pattern #85 N=1)
...
#### ✅ #83 Honest-Deficiency-Disclosure Discipline → PROMOTED to CONFIRMED at v69 OVERDUE-NATURAL-CADENCE mini-audit (2026-05-19)

**Status:** PROMOTED to CONFIRMED at v69 audit under criterion 1 (≥3-cross-tier, N=5 evidence v64+v65+v67+v68+v69 with HIGH confidence). Full confirmed entry: `_patterns/02b-confirmed-patterns-v42-plus.md` § "Pattern #83 — promoted at v69 OVERDUE-NATURAL-CADENCE mini-audit". Sub-taxonomy 83a / 83b / 83c / 83d / 83e registered at promotion.

**Status path:** v66 EARLY mini-audit registration N=2 stale-risk-flagged → v67 N=3 PROMOTION-ELIGIBLE (proposal-document filed) → v68 N=4 consolidation → v69 N=5 consolidation with 5-disclosure-surface saturation at CloakBrowser → **v69 OVERDUE-NATURAL-CADENCE audit PROMOTION**.
...
**Original candidate entry preserved below for narrative continuity:**

#### 🟡 #83 Honest-Deficiency-Disclosure Discipline (NEW v66 EARLY mini-audit — registered N=2 stale-risk-flagged) [PROMOTED v69]
...
#### ✅ #84 Cross-Vendor Ecosystem-Tolerance → PROMOTED to CONFIRMED at v72 PROMOTION-DRIVEN + 5TH-CONSECUTIVE-TRIGGER mini-audit (2026-05-19)

**Status:** PROMOTED to CONFIRMED at v72 audit under criterion 1 (≥3-cross-tier, N=3 evidence v62+v65+v71) + criterion 4 (variant-within-pattern, 3 sub-mechanisms 84a/84b/84c) with HIGH confidence. Full confirmed entry: `_patterns/02b-confirmed-patterns-v42-plus.md` § "Pattern #84 — promoted at v72 PROMOTION-DRIVEN + 5TH-CONSECUTIVE-TRIGGER mini-audit". Sub-taxonomy 84a Tool-tolerance / 84b Usage-enforcement / 84c Provider-Agnostic-By-Design (NEW) registered at promotion.

**Status path:** v66 EARLY mini-audit registration N=2 stale-risk-flagged → v67 84a/84b refinement → v71 N=3 PROMOTION-ELIGIBLE with 84c proposal-document → **v72 PROMOTION-DRIVEN + 5TH-CONSECUTIVE-TRIGGER audit PROMOTION**.
...
#### 🟡 #84 Cross-Vendor Ecosystem-Tolerance (NEW v66 EARLY mini-audit — registered N=2 stale-risk-flagged) [PROMOTED v72]
...
#### 🟡 #85 Platform-Primitive Foundation (NEW v66 agentmemory wiki — registered N=1 stale-flagged)
**Status at v66 agentmemory wiki (2026-05-14):** Registered as candidate — PRIMARY Phase 4b deliverable of the v66 wiki. **N=1 stale-flagged at registration.** Corpus-first observation across 65 wikis. Full registration proposal: `03 Projects/agentmemory - Beginner Analysis/01 Analysis/(C) Pattern Platform-Primitive Foundation N=1 registration proposal.md`.
**Evidence:** v66 agentmemory only — `rohitg00/agentmemory` is built ENTIRELY on the iii-engine platform's primitives. README states verbatim: *"agentmemory is already a running iii instance. It uses iii primitives exclusively."* All infrastructure delegated: HTTP Triggers (replaces Express/Fastify) / KV State (replaces SQLite/Postgres+pgvector) / Streams (replaces SSE/Socket.io) / Worker Supervision (replaces pm2/systemd) / OTEL Integration (replaces Prometheus/Grafana) / Function Registry (replaces custom plugin systems). `package.json` has 6 runtime deps, none an HTTP framework or DB driver — entire infra stack is the single `iii-sdk ^0.11.2` line. Self-counts in host units ("123 functions · 34 KV scopes"). Extended via host CLI (`iii worker add iii-pubsub / iii-cron / iii-queue / iii-sandbox`), not its own plugin system. Coupling-risk materialized: `iii-sdk` pinned to v0.11.2 because v0.11.6+ broke the project (CHANGELOG: "users on v0.11.6+ must downgrade until refactor completes").
**Formal statement (candidate):**
> A framework or service is built ENTIRELY on another platform's primitives — delegating ALL infrastructure concerns (HTTP serving / storage / streams / process supervision / observability / extension system) to the host platform — and signals this structurally by (a) self-identifying as "a running instance" of the host, (b) counting its own internals in the host's units, and (c) exposing extensibility through the host's CLI rather than its own plugin system. Distinct from ordinary framework-dependency by the EXCLUSIVITY of the delegation and the SELF-AS-INSTANCE framing. Carries intrinsic coupling-risk: a breaking change in the foundation hits the instance across its whole substrate at once.
**Promotion criteria (4 — all 4 required per subject to count as evidence):**
1. Exclusive infrastructure delegation — no self-rolled HTTP framework / DB-storage layer / stream layer / process supervisor / observability stack
2. Self-as-instance framing — the project describes itself as BEING an instance of the host, not merely USING it
...
**Required for promotion:** 2+ subjects meeting all 4 criteria.
**Re-evaluation:** v68 stale-check / v71 retire-check.
...
### Pattern #84 update at v67 — REFINEMENT EVIDENCE (not strengthening) [PROMOTED v72]
...
#### 🟡 #86 T1 sub-archetype #7 "programming-language-as-agent-substrate" (NEW v68 zero wiki — registered N=1 PROVISIONAL stale-risk-flagged)
**Status at v68 wiki ship (2026-05-18):** Registered as candidate via vercel-labs/zero v68 first-corpus programming-language subject. **N=1 PROVISIONAL T1 NEW sub-archetype #7 with explicit v69 Tier-Taxonomy Review T6 audit-decision flag** for NEW Tier T6 Language/Runtime substrate vs T1 sub-archetype retention. **Corpus-first programming-language subject in 67-prior-wiki history.**
**Evidence:** v68 vercel-labs/zero only — "agent-first programming language" published by Vercel Labs (sub-org of Vercel main; v51 corpus subject parent); 2-day-old project at wiki build (v0.1.0 2026-05-16); EXTREME-VIRAL 1,050 stars/day (corpus 2nd-highest); 5 explicit design axioms (agent-first learnability + standard-library depth + deterministic tooling + direct DX + regularity-over-syntax); self-hosted compiler bootstrap (`native/zero-c/` + `compiler-zero/`); skill content bundled in compiler binary served via `zero skills get zero --full` (Skill-As-Binary-Bootstrap mechanism); 7+ inner skills (zero-agent / zero-language / zero-diagnostics / zero-packages / zero-builds / zero-testing / zero-stdlib); Rust-style borrow checking; in-browser WASM compiler playground at zerolang.ai. Pattern #83 N=4 strengthening evidence (Pre-1.0 honest disclosure: "Security vulnerabilities should be expected" + "Not ready for production").
**Formal statement (candidate):**
...
**Promotion criteria (4 — all 4 required per subject):**
1. Primary artifact is a programming language with its own compiler/runtime (not skill collection / service / plugin)
2. Agents are PRIMARY users (explicit in design documentation; not augmentees)
3. Language-level commitments to agent-readability (design axioms enforcing structural choices: regularity-over-syntax / standard-library-depth / structured-output)
...
**Rationale:** Corpus-first across 67 prior wikis. Distinguishing from existing T1 sub-archetypes (general-purpose / curated-meta / corporate-curated / single-artifact / domain-vertical / reverse-engineering-reference-archive) is structurally clean — Zero's primary artifact is a language with compiler infrastructure, not a skill collection or curation. Hypothetical un-stale candidates: Anthropic-published "agent DSL" / OpenAI Codex-derived agent language / independent solo programming language with agent-first framing / existing language adopting agent-first reframing (Python 4 / Rust 2) / Vercel Labs successor projects. Stale-risk: may stay N=1 if "programming language with agent-first explicit framing" remains rare; the narrower trait (Skill-As-Binary-Bootstrap mechanism) may generalize faster than the full sub-archetype.
**Cross-reference:** Pattern #77 T1 sub-archetype tracking (active candidate registry for T1 sub-archetypes); Pattern #2 Distribution Evolution (Skill-As-Binary-Bootstrap mechanism could be Pattern #2 sub-variant); Pattern #57 corpus-citation chain (Anthropic skills protocol corpus-confirmed); v66 Decision 14 deferral of Tier-Taxonomy Review T6 (formalized at v68). **v69 Tier-Taxonomy Review T6 audit-decision: ADOPT T1 sub-archetype #7 + DEFER NEW Tier T6 decision to future audit when N=2 emerges.**
**See full proposal:** `03 Projects/zero - Beginner Analysis/01 Analysis/(C) T1-programming-language-as-agent-substrate NEW sub-archetype registration.md`.

---

#### 🟡 #87 Skill-As-Binary-Bootstrap (NEW v68 zero wiki — registered N=1 observational candidate)
**Status at v68 wiki ship (2026-05-18):** Registered as observational candidate via vercel-labs/zero v68. **N=1 observational-track-flagged at registration.** Could be Pattern #2 Distribution Evolution sub-variant OR top-level pattern; audit decides at v69+.
**Evidence:** v68 vercel-labs/zero only — `skills/zero/SKILL.md` is thin discovery stub (51 lines) using Anthropic skills protocol frontmatter (name + description). Stub explicitly states: "This file is a discovery stub. Do not treat it as the full Zero workflow." Stub directs agents to invoke `zero skills get zero --full` on installed compiler. Compiler binary serves version-matched skill content from bundled `skill-data/` (7+ inner skills: zero-agent / zero-language / zero-diagnostics / zero-packages / zero-builds / zero-testing / zero-stdlib). Multi-binary-coherence-as-skill-routing-concern: "If the user has multiple Zero binaries, use the same binary that will run the project."
**Formal statement (candidate):**
...
**Rationale:** Corpus-first mechanism solving observable skill-content-version-drift problem. Distinct from MCP runtime (v66 agentmemory — different protocol) and from static skill packaging (corpus default). Could be adopted by other runtimes (other compilers, agent runtimes, plugin systems). Library-vocabulary candidate item alternative: if mechanism doesn't reach N=2 by v70, demote to Library-vocabulary observational item.
**Cross-reference:** Pattern #2 Distribution Evolution (CONFIRMED) — Skill-As-Binary-Bootstrap is candidate sub-variant of Pattern #2's packaging-channel evolution. Could register as Pattern #2 sub-variant rather than top-level pattern. Pattern #57 corpus-citation chain (Anthropic skills protocol corpus-confirmed; v68 strengthens corpus-citation observation). v66 agentmemory N=15+ runtime-served skills via MCP — adjacent mechanism with different protocol (MCP vs skill protocol).

---

#### 🔍 OC-A Agent-First-Language-Substrate design philosophy (NEW v68 zero wiki — registered N=1 observational track)
**Status at v68 wiki ship (2026-05-18):** Registered as observational track via vercel-labs/zero v68. **N=1 stale-flagged at registration.**
**Evidence:** v68 vercel-labs/zero only — explicit "agents are primary users from day one" framing in README + AGENTS.md. 5 supporting design axioms (agent-first learnability / standard-library depth / deterministic tooling / direct DX / regularity-over-syntax). Inverts traditional human-first language design — agents are END USER, not augmentee. Distinct from agent-augmentation pattern (prior corpus default where agents augment human users via skill collections).
**Re-evaluation:** v70 stale-check / v73 retire-check.
**Rationale:** Could generalize if other languages adopt "agent-first" framing as design axiom. May stay corpus-N=1 if framing remains Vercel Labs-specific. Sister to Sub-archetype #7 registration (Pattern #86); #86 is the structural classification; OC-A is the design-philosophy observation.

---

#### 🔍 OC-B Self-Hosted Compiler Bootstrap (NEW v68 zero wiki — registered N=1 observational track)
**Status at v68 wiki ship (2026-05-18):** Registered as observational track via vercel-labs/zero v68. **N=1 stale-flagged at registration.**
**Evidence:** v68 vercel-labs/zero only — `native/zero-c/` (C bootstrap compiler) + `compiler-zero/` (Zero-authored self-hosted compiler). Native bootstrap remains user-installed; compiler-zero/ compiles to WASM for browser playground. Classic bootstrapped-compiler pattern, corpus-first as wiki subject.
**Re-evaluation:** v70 stale-check / v73 retire-check.
**Rationale:** Pattern is well-known in compiler engineering but corpus-N=1 because most corpus subjects aren't compilers. Likely to stay N=1 unless other compiler subjects enter corpus.

---

#### 🔍 OC-C In-Browser-Compiler-Via-WASM (NEW v68 zero wiki — registered N=1 observational track)
**Status at v68 wiki ship (2026-05-18):** Registered as observational track via vercel-labs/zero v68. **N=1 stale-flagged at registration.**
**Evidence:** v68 vercel-labs/zero only — `compiler-zero/` compiles to WASM (`wasm32-web` target) for docs-site browser playground at zerolang.ai. Users can run Zero in-browser without installing. Browser-compiler-hardening scripts ensure WASM runtime safety.
**Re-evaluation:** v70 stale-check / v73 retire-check.
**Rationale:** Could generalize to other language subjects in browser-playground contexts. Sister to OC-B Self-Hosted Compiler Bootstrap (uses self-hosted compiler as the WASM target).

---

#### 🔍 OC-D Anti-Compatibility-As-Project-Policy (NEW v68 zero wiki — registered N=1 observational track)
**Status at v68 wiki ship (2026-05-18):** Registered as observational track via vercel-labs/zero v68. **N=1 stale-flagged at registration.**
**Evidence:** v68 vercel-labs/zero only — AGENTS.md explicit: "Do not preserve legacy behavior by default. Prefer the clearer agent-facing design over compatibility shims, migration layers, or carrying old paths forward." Deliberate anti-compatibility stance; breaking changes are the DEFAULT.
**Re-evaluation:** v70 stale-check / v73 retire-check.
**Rationale:** Distinct counter-norm to typical software-engineering norms (which value compatibility). Could observe at other pre-1.0 / experimental projects. Sister observation to v62 codex-plugin-cc (compatibility-preserving) + v66 agentmemory (versioned migrations) + v67 opencode-antigravity-auth (Storage V4 migration chain) — all corpus subjects that ARE compatibility-aware vs Zero's inversion.

---

#### 🔍 OC-E Synchronicity-Discipline-As-Policy (NEW v68 zero wiki — registered N=1 observational track)
**Status at v68 wiki ship (2026-05-18):** Registered as observational track via vercel-labs/zero v68. **N=1 stale-flagged at registration.**
**Evidence:** v68 vercel-labs/zero only — AGENTS.md explicit: "Keep examples, docs, tests, and command contracts aligned with the new behavior so the repository describes one coherent current system." BEHAVIORAL alignment across repository surfaces (distinct from Pattern #81 version-string consistency).
**Re-evaluation:** v70 stale-check / v73 retire-check.
**Rationale:** Distinct from Pattern #81 Manifest-Drift (which is version-string consistency). OC-E is BEHAVIORAL synchronicity. Could generalize to other strict-discipline projects. Sister observation flag at v68 audit.

---

#### 🔍 OC-F Structured-JSON-as-First-Class-CLI-Output (NEW v68 zero wiki — registered N=1 observational track)
**Status at v68 wiki ship (2026-05-18):** Registered as observational track via vercel-labs/zero v68. **N=1 stale-flagged at registration.**
**Evidence:** v68 vercel-labs/zero only — 5 of 8 Zero common commands have `--json` flag (`zero check --json` / `zero graph --json` / `zero size --json` / `zero routes --json` / `zero fix --plan --json` / `zero doctor --json`). Machine-readable output as default, not secondary `--format=json`. Aligns with "deterministic tooling" design axiom.
**Re-evaluation:** v70 stale-check / v73 retire-check.
**Rationale:** Observable across recent corpus tooling but Zero v68 formalizes the pattern. Could promote to Pattern candidate if N=2+ subjects emerge with explicit --json-as-default discipline.

---

### Pattern #83 update at v68 — N=4 strengthening (consolidation, not promotion-change)

**v68 evidence (2026-05-18):** vercel-labs/zero N=4 STRONG via 2 distinct disclosure surfaces saturated:
1. README opens with explicit pre-1.0 + security disclosure: "Zero is pre-1 and intentionally unstable... Security vulnerabilities should be expected. Zero is not ready for production systems, sensitive data, or trusted infrastructure."
2. AGENTS.md §Safety repeats + adds specific failure modes: "Run and develop Zero in safe environments: isolated workspaces, disposable inputs, and systems where compiler crashes, malformed output, or unsafe runtime behavior cannot damage production state."

All 3 registration criteria SATISFIED with STRONG PASS verdicts. Same archetype as v67 (legal/operational + categorical disclosure of failure modes).

**Status:** Pattern #83 remains CANDIDATE N=4 PROMOTION-ELIGIBLE. **v68 evidence consolidates v67 N=3 promotion case; no acceleration request. Promotion target at v68 natural-cadence audit / v69 audit window per v67 proposal.**
...
### Pattern #52 update at v68 — v69 sustained-velocity-test batch addition

**v68 evidence:** vercel-labs/zero at 1,050 stars/day in 2-day-old window = **corpus 2nd-highest velocity in 67-wiki corpus history**, only karpathy-skills v63 (~1,186/day at 102 days) was higher.

**Status:** ADD v68 zero to v69 Pattern #52 sustained-velocity-test batch alongside mattpocock + codex-plugin-cc + karpathy-skills + agentmemory. **v68 zero is unique test case** — corporate-strategic-lab launch at 2-day-old (vs other batch subjects at 30+ days).

**Special interest:** v69 sustained-velocity result will indicate whether corporate-lab launches sustain velocity differently from solo-distillation / cross-vendor-cooperation source archetypes.

---

### Pattern #81 update at v68 — partial N=2 evidence (sister observation flag)

**v68 evidence:** Zero AGENTS.md release workflow §"Bump the release version in `package.json`, `package-lock.json`, `docs-site/package.json`, `extensions/vscode/package.json`, and `native/zero-c/src/main.c`" = 5-file version-bump discipline (matches Pattern #81 structural property: ≥3 file locations).

**Critical distinction:** Workflow described as MANUAL ("Releases are manual, single-branch affairs"). NOT CI-enforced.

Pattern #81 promotion criteria require: (1) CI explicitly asserts version-string consistency; (2) Drift causes mechanical PR-block. Zero v68 satisfies (3) "Framework documents this as discipline" but FAILS (1) and (2) CI-enforcement.

**Status:** **Sister observation flag** — Pattern #81 "discipline-documented-but-not-CI-enforced sub-variant" registered for v68 audit consideration. NOT direct N=2 promotion evidence; rather Pattern #81 needs sub-typology decision: 81a CI-enforced (v64 claude-seo baseline) vs 81b manually-disciplined (v68 zero) sub-variants.

---

### Pattern #82 update at v68 — WEAK N=2 (sister observation, different mechanism)

**v68 evidence:** Zero CHANGELOG lists named contributors per release (`@ctate @badlogic @chenrui333 @heylakatos @onevcat` for v0.1.2; `@ctate @mvanhorn` for v0.1.1). Per-release attribution discipline.
...
### Pattern #84 update at v71 — N=3 PROMOTION-ELIGIBLE + sub-typology 84c proposed [PROMOTED v72]
...
### Pattern #52 update at v72 — HIGH-NOT-EXTREME sub-class N=2 PROMOTION-ELIGIBLE

**v72 wiki context (2026-05-19):** v72 audit (same day, morning) registered Pattern #52 3-tier velocity sub-classification (EXTREME-VIRAL >300/d + **HIGH-NOT-EXTREME 150-300/d** + SUSTAINED-MODERATE-HIGH 25-150/d) with top-level promotion DEFERRED to v75 awaiting "N=2 within HIGH-NOT-EXTREME or SUSTAINED-MODERATE-HIGH sub-classes." **v72 wiki (this entry) satisfies the v72 audit's deferred-promotion criteria with 0-wiki gap = CORPUS-FIRST audit-policy-satisfaction speed.**

**v72 evidence:** Hmbown/DeepSeek-TUI = 267.6 stars/day × 120 days = N=2 within HIGH-NOT-EXTREME sub-class.

**HIGH-NOT-EXTREME sub-class N=2 (cross-tier + cross-archetype):**
...
**Promotion criteria evaluation (sub-variant level):**
- ✅ Criterion 1: N=2 across T2 + T1 (cross-tier)
- ✅ Criterion 4 (variant-within-pattern-at-N=2): HIGH-NOT-EXTREME is formal sub-variant registered at v72 audit; N=2 satisfies criterion 4 STRONG PASS
- ✅ Cross-archetype: corporate-anonymous + solo-community (2 distinct archetypes)

**Promotion verdict:** Pattern #52 **PROMOTION-ELIGIBLE at v72** via cumulative sub-variant evidence; **DEFER formal top-level promotion to v75 audit** per v72 audit's deferred-promotion schedule (audit-velocity-control discipline).

**v75 audit path:** Pattern #52 PROMOTE to CONFIRMED with 3 sub-variants registered (52d EXTREME-VIRAL + 52e HIGH-NOT-EXTREME + 52f SUSTAINED-MODERATE-HIGH).
...
**Library-vocabulary item #10 N+1 (1-wiki Rapid-Pattern-Evolution):** v72-audit-sub-classification-registration → v72-wiki-N=2-evidence-within-sub-classification = **0-wiki gap = CORPUS-RECORD at sub-class layer**. Library-vocabulary item #10 N=5.

---

### Pattern #84 update at v72 — FIRST POST-PROMOTION 84c STRENGTHENING

**v72 wiki context:** Pattern #84 PROMOTED to CONFIRMED at v72 audit (same day, morning) with sub-taxonomy 84a/84b/**84c Provider-Agnostic-By-Design**. Audit warned: "If any sub-mechanism fails to reach N=2 within 4 wikis post-promotion (v76), reformulate Pattern #84 to retire the unsupported sub-mechanism."

**v72 evidence (this wiki):** DeepSeek-TUI 9-provider matrix:

| # | Provider | Type |
|---|----------|------|
| 1 | DeepSeek (default) | Hosted (primary vendor) |
| 2 | NVIDIA NIM | Hosted |
| 3 | AtlasCloud | Hosted |
| 4 | OpenRouter | Hosted (aggregator) |
| 5 | Novita | Hosted |
| 6 | Fireworks | Hosted |
| 7 | Generic OpenAI-compatible | Hosted (via `OPENAI_BASE_URL`) |
| 8 | SGLang | Self-hosted |
| 9 | vLLM | Self-hosted |
| 10 | Ollama | Self-hosted |

**Sub-mechanism 84c quantitative escalation:**
- v71 evidence: dual-vendor synthesis (Anthropic 5-guide + OpenAI 5-guide corpora)
- v72 evidence: 9-provider matrix (DeepSeek + 8 alternatives) — quantitative escalation within 84c

**Significance:** **FIRST POST-v72-AUDIT 84c STRENGTHENING IN CORPUS HISTORY** — sub-mechanism N=2 reached at v72 wiki within 0 wikis of audit registration. **Fastest sub-mechanism N=2 in corpus history.** RELIEVES Pattern #84 v76 retire-check threat.

**Sub-typology candidate:** 84c could split into 84c1 "dual-vendor-synthesis" + 84c2 "n-provider-matrix" at v75+ audit if more N+1 evidence emerges. Currently consolidated under 84c.

---

### Pattern #21 update at v72 — Sub-variant candidate "Session-Longevity-Discipline-As-Pattern"

**v72 evidence:** Hmbown/DeepSeek-TUI AGENTS.md "Session Longevity (Critical)" section provides 6-rule discipline:

1. **Delegate independent work early** via `agent_open` — bounded sub-agent sessions for non-blocking parallel work
2. **Batch independent reads/searches** — fire together; summarize evidence instead of repeated tool rows
3. **Compact aggressively** — `/compact` at 60% not 80%
4. **Reassess after 3 sequential parent turns** — split into sub-agents or RLM sessions
5. **Use RLM for batch classification** — focused Python + `sub_query_batch` instead of repeated parent reads
6. **After every 3 turns, check:** context under 60%? sub-agents still running? `cargo check` still passes?

**Operating model:** Keep parent session lean; large-context inspection in RLM; parallel side work in sub-agents; full outputs behind handles/detail pagers; only decision-quality summary in main thread.

**Distinct from v71's 7-invariant loop:** v71 architects execution-separation principle (model-proposes-harness-disposes); v72 architects **operator-context-management principle** at complementary layer.

**Sub-variant candidacy:** Register at v75 audit as Pattern #21 sub-variant "Session-Longevity-Discipline-As-Pattern" or absorb as 21c sub-mechanism (continuous-context-management).

---

### Pattern #66 update at v72 — sub-mechanism 66e candidate "GitHub-issue-as-prompt-injection-attack-surface"

**v72 evidence:** AGENTS.md "Watch for issue / PR injection" section — corpus-first explicit GitHub-artifacts-as-untrusted-input formal treatment:
...
- External branding / logos require explicit maintainer approval
- Promotional language gets cut on review

**Sister to 66d (skill-package layer):** Both 66d (v71 agents-best-practices) and 66e (v72 DeepSeek-TUI) are community-input-layer supply-chain attack surfaces. Could form N=2 within Pattern #66 community-input-layer family at v75 audit.

**Provisional registration:** REGISTER 66e PROVISIONAL at v75 audit; if v75+ adds N=2 within issue-injection-layer, promote 66e to formal sub-mechanism.

---

### Pattern #18 update at v72 — sub-mechanism C candidate "dispatcher-companion-binary-split"

**v72 evidence:** Hmbown/DeepSeek-TUI 2-binary architecture:
- `deepseek` (CLI dispatcher, `crates/cli`)
- `deepseek-tui` (companion ratatui binary, `crates/tui`)

Ships as SEPARATE executables. Dispatcher resolves and spawns `deepseek-tui` as sibling on PATH at runtime.

**Distinct from sub-mechanism B (shared-backend-service):**
- B = one-binary-many-CLIENTS (server-architecture via MCP/CDP)
- C = dispatcher-companion-binary-split (client-architecture; one user-facing brand, separate execution units for update-path isolation + interactive-vs-headless surface separation)

**N=1 at v72.** Provisional registration; promote to formal sub-mechanism at N=2 if v75+ adds dispatcher-companion architecture subject.

**4-layer hierarchy implication:** If sub-mechanism C reaches N=2 and additional protocol-variant evidence emerges, Pattern #18 may extend to 5-layer hierarchy (top-level → sub-archetype → sub-mechanism → protocol-variant → architecture-variant).

---

#### 🔍 OC-P — Community-Built-Canonical-Vendor-Client (NEW v72 DeepSeek-TUI wiki — registered N=1 observational candidate)

**Status at v72 wiki (2026-05-19):** Registered as observational candidate via DeepSeek-TUI corpus-first community-built-canonical-vendor-client observation. **N=1 stale-flagged at registration**.

**Evidence:** Hmbown/DeepSeek-TUI — solo individual developer (Hmbown handle; pseudonymous-likely; no DeepSeek employee verification) builds vendor-product-quality terminal coding agent for DeepSeek API. 32,114 stars at 120 days = vendor-employee-product scale. Community-aligned with DeepSeek-China-market positioning (Tencent Cloud / CNB integration). Provides canonical client experience for DeepSeek API; multi-provider matrix (9 providers) demonstrates portability.

**Formal statement (candidate):**
> Solo individual developer builds vendor-product-quality terminal/desktop client for a specific vendor's API series, becoming de-facto canonical client (32K+ stars at vendor-employee-product scale) while remaining independent from vendor employment/sponsorship. Client provides best-experience FOR vendor's API while also supporting other vendor APIs (multi-provider matrix); distinct from REVERSED (vendor publishes for rival) / REVERSE-ENGINEERING-ARCHIVE / PROVIDER-AGNOSTIC-SYNTHESIS sub-archetypes.

**Promotion criteria (4):**
1. Solo individual developer (not corporate team) builds client
2. Client reaches vendor-employee-product velocity / star count (rough: ≥10K stars or ≥100/day sustained)
3. Multi-provider support included alongside vendor-primary support (provider-agnostic-by-design at architecture layer)
4. No vendor employment / sponsorship verification (community-built, not vendor-paid)

**Required for promotion:** 2+ subjects with all 4 criteria.

**Re-evaluation:** v75 first-cycle stale-check / v77 retire-check.

**Rationale:** Distinct from prior corpus subjects:
- v62 codex-plugin-cc — REVERSED (vendor publishes FOR rival platform) — Pattern #84 84a tool-tolerance
- v65 claude-code-system-prompts — REVERSE-ENGINEERING ARCHIVE (Piebald-AI documents Anthropic internals) — Pattern #83 + Pattern #84 84b usage-enforcement
- v71 agents-best-practices — PROVIDER-AGNOSTIC SYNTHESIS (DenisSergeevitch builds dual-vendor framework; not canonical client for one vendor) — Pattern #84 84c
- **v72 DeepSeek-TUI — community-built CANONICAL client (best-experience FOR vendor, while ALSO supporting other vendors) = corpus-first**

**Hypothetical un-stale subjects:** Other community-built canonical clients for vendor APIs reaching vendor-employee-product scale (e.g., OpenAI-canonical TUI, Anthropic-canonical TUI, Gemini-canonical TUI — none currently identified in corpus).

**Cross-reference:** Pattern #84 84c is COMPATIBLE with OC-P (DeepSeek-TUI exemplifies BOTH — vendor-canonical-client AND 9-provider matrix). Could converge into 84c sub-typology at v77+ if OC-P reaches N=2.

**Distinguishing from existing patterns:**
- Pattern #19 (Ecosystem-Portfolio-Builder, CONFIRMED) — about builders with multiple products; OC-P is about a single product reaching vendor-employee-product scale
- Pattern #57 (Author-Authority-Through-Citation, CONFIRMED) — about citation discipline; OC-P is about product-scale-velocity from community-build
- Pattern #84 84c (Provider-Agnostic-By-Design, CONFIRMED v72) — overlaps but 84c emphasizes author design choice for multi-vendor; OC-P emphasizes community-built-canonical-for-single-vendor with multi-vendor as secondary

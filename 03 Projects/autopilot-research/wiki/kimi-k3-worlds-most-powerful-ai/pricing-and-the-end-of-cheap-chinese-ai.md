# Pricing — and "the end of super cheap Chinese AI"

The video (~[10:37]–[14:18]) frames pricing as K3's killer feature: "very, very cost-effective … won't break the bank." The rate card is right; **the framing is backwards.**

## The rate card (CONFIRMED)

- **$3.00 / M input** (cache-miss), **$0.30 / M** cache-hit input, **$15.00 / M output**.
- **Cost per Intelligence-Index task ≈ $0.94** vs **Opus 4.8 ≈ $1.80** (about half) and **GPT-5.6 Sol ≈ $1.04** (comparable). CONFIRMED via Artificial Analysis.
...
## The verbosity tax (hidden cost)

- On the AA Intelligence Index, K3 generated **130M output tokens vs a 63M median** — it's **~2× as verbose** as comparable models. Because output is the expensive side ($15/M), the **effective per-task cost runs higher than the rate card implies** for output-heavy workloads. Rate-card cheapness ≠ real-workload cheapness.

## Context: it's not the *whole* Chinese ecosystem

- "End of cheap Chinese AI" refers to **frontier/flagship** models from major labs. Budget options (DeepSeek, Doubao) still exist far cheaper (~$0.04/task tier). K3 is specifically the **most expensive Chinese-lab frontier model to date** — a positioning pivot, not an industry-wide price floor.

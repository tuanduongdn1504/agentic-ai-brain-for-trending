...
- A memory store is "a workspace-scoped collection of **text documents**… mounted as a directory inside the session's sandbox. The agent reads and writes it with the same file tools it uses for the rest of the filesystem" (mount at `/mnt/memory/<slug>/`; ≤100KB per memory; **2,000 memories/store; 8 stores/session**; immutable per-mutation versions with redact, 30-day retention).
- **Dreams — Anthropic's literal consolidation agent**: "A dream reads an existing memory store alongside past session transcripts, then produces a new, reorganized memory store: **duplicates merged, stale or contradicted entries replaced with the latest value, and new insights surfaced**." Trigger = **explicit API call**, not automatic (press "scheduled process" language = usage pattern, not API property). Announced Code with Claude SF **2026-05-06**; gated **research preview** inside the CMA beta; CMA is stateful server-side → **not ZDR-eligible**.
- **→ Full first-party treatment (2026-07-04 deepening): [[anthropic-memory-stores-and-dreaming]]** — limits tables, dream lifecycle/models/billing, workshop-only harness internals, the index-file demo, and the cost-economics claims with their evidence tiers.
...
## ⚠️ "AutoDream" — multi-third-party, still zero official docs (status upgraded 2026-07-04)

Third-party coverage of a Claude Code "AutoDream" auto-consolidation cycle (≥24h + ≥5 sessions or manual `/dream`; Orient→Gather→Consolidate→Prune; <25KB output; "shipped ~2026-03 v2.1.83+ behind a server-side feature flag") has grown from single-source (zenvanriel.com) to **multiple converging third-party sources** (claudefa.st — which publishes what it claims is the internal system prompt, tessl.io, EasyClaw, dev blogs, replica skill repos). **Official docs still contain nothing**: code.claude.com/docs/en/memory (re-fetched 2026-07-04) has zero dream/consolidation mentions; anthropics/claude-code release notes likewise; Anthropic uses "dreaming" officially ONLY for Managed Agents. "AutoDream" is a **community-coined name**. Two cautions: (1) the third-party posts show content-replication chains — multi-source ≠ independent; (2) do NOT conflate it with Managed Agents Dreams — **opposite trigger models** (auto/feature-flagged vs explicit API). Status: reported-only, now multi-source; still not citable as fact. Details in [[caveats-and-corrections]].

...
- **Both vendors converged on "dreaming."** OpenAI shipped "Dreaming V3" (2026-06-04, vendor-reported 9.4%→75.1% time-sensitive accuracy) and Anthropic ships a "Dreams" feature for Managed Agents (reads transcripts + memory store → reorganized store). Background consolidation by a separate process is now the production norm — the video's core intuition is right.

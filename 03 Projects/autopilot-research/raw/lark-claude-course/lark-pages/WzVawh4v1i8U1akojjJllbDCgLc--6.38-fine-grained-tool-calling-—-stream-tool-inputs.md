...
**Source:** https://transform.sg.larksuite.com/wiki/WzVawh4v1i8U1akojjJllbDCgLc
**Wiki ID:** WzVawh4v1i8U1akojjJllbDCgLc
**Archived:** 2026-05-07
...
Log In or Sign Up
6.38 - Fine-grained tool calling — Stream tool inputs
Bài 6.38: Fine-grained tool calling — Stream tool inputs
Mục tiêu học tập
Mở đầu: Tool streaming cần nhanh hơn
...
❌ Don't use when
Code example
UI pattern: Progressive tool call display
Validation vẫn happen cuối cùng
Combined with text streaming
Anti-patterns
...
Streaming tool = Claude gửi partial JSON args từng chunk, bạn hiển thị progress cho user.​
Nhưng mặc định Anthropic validate JSON trước khi send → có delay. Fine-grained mode skip validation → stream faster.​
Basic tool streaming​

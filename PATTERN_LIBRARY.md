...
## Current state (post-v68 vercel-labs/zero wiki 2026-05-18)

- **Confirmed:** **43** (v66 EARLY-OPERATOR-ELECTED audit promoted Pattern #19 ecosystem-portfolio-builder N=4 + Pattern #78 Living-Domain-Standards Tracking; **Pattern #83 N=4 PROMOTION-ELIGIBLE at v68 natural-cadence audit — HIGH confidence, consolidated v64+v65+v67+v68 evidence**)
- **Active candidates:** **31** (v66 audit registered #80-#84; v66 agentmemory wiki registered #85; v67 opencode-antigravity-auth wiki registered 3 NEW OCs OC-1/OC-2/OC-3 + Library-vocabulary item #11; **v68 zero wiki registered NEW Pattern #86 T1 sub-archetype #7 "programming-language-as-agent-substrate" + NEW Pattern #87 Skill-As-Binary-Bootstrap observational + 6 OCs OC-A through OC-F**)
- **Stale candidates:** **1** (Pattern #52 Extreme-Viral-Velocity OBSERVATIONAL FLAG preserved)
- **Retired:** 9
- **Observation-tracks:** 6 (formal OTs) + 13 NEW observational candidates accumulated v67+v68 (Skill-As-Binary-Bootstrap + OC-A through OC-F at v68 + OC-1 through OC-3 at v67)
- **Total:** **97 full / 74 active-counting** (confirmed + active)
- **Ratio:** 31:43 = **0.721:1** — buffer **0.229** below the 0.95:1 mini-audit trigger
- **Library-vocabulary items:** 14 (was 13; v67 added item #11 Engagement-Deficit-Extreme — corpus-record fork_ratio 0.0001)
- **Routine:** `llm-wiki-routine-v2.2` (codified 2026-05-14; **v66 + v67 + v68 all under v2.2 — 3 wikis + 1 audit validated; all 9 v2.2 features exercised cleanly**)
- **Storm Bear meta-entity streak:** **4 consecutive PASS post-v64-RESET** (v65 STRONGEST 3/4 → v66 STRONG 4/4 → v67 MODERATE 2/4 → **v68 WEAK 1/4 — FIRST WEAK in post-amendment window**); **strength categorization spectrum fully exercised at 4 consecutive wikis** (STRONGEST/STRONG/MODERATE/WEAK) — v2.2's strength categorization (NEW per v65 lesson) **discriminates fully across distinct relevance levels — validated empirically at v68**; 12-instance window v57-v68 = 10 PASS / 2 SKIP = **83.3% INCLUDE rate**
- **Active candidate count trigger:** REACHED at v67 + maintained at v68 (31 candidates ≥ 30 trigger) — **mini-audit zone met via candidate-count metric** (ratio still 0.721 well below 0.95); **v68 audit batch ~20 items = LARGEST in corpus history**
- **Authoritative sources:** `_patterns/02a-confirmed-patterns-pre-v42.md` + `_patterns/02b-confirmed-patterns-v42-plus.md` (confirmed) / `_patterns/03-active-candidates.md` (31 candidates incl. #85 + #86 + #87 + OC-1-through-OC-3 + OC-A-through-OC-F) / `_patterns/04-retired-stale.md` / `_patterns/05-recent-additions.md` (v54→v68 strengthening + audit blocks) / `_patterns/01-audit-history.md` (audit history v21-v55) / root `CLAUDE.md` (compact current-state block)

### v60 → v68 narrative arc
...
**Honest disclosure on ratio drift:** the ratio rose 0.415 → 0.721 across v60→v68. Of the +0.306 drift: v66 audit disposed the v64+v65 observational queue (5 NEW candidates + 1 wiki-ship) for +0.213; v67 added 3 NEW observational candidates for +0.070; v68 added 1 NEW Pattern (#86 T1 sub-archetype) + 1 NEW Pattern (#87 Skill-As-Binary-Bootstrap) + 6 NEW OCs for +0.023. Real observations from genuinely novel subjects (FIRST programming-language in corpus), not a discipline regression. The library held the sub-0.55 zone v60-v64; v66-v68 operate outside it but within healthy bounds (buffer 0.229 below the 0.95:1 trigger). **Active count trigger REACHED at v67 + maintained at v68 (31 candidates) — v68 mini-audit zone fully met via candidate-count metric** (ratio remains well below at 0.721). **v68 audit batch ~20 items = LARGEST in corpus history.** Forecast: drift back toward sub-0.55 by v70-v72 absent extreme novelty, accelerated by v68 audit disposition.
...
| `_patterns/03-active-candidates.md` | 31 active candidates with formal statements + observations + N tracking (incl. #85 Platform-Primitive Foundation + #86 T1 sub-archetype #7 + #87 Skill-As-Binary-Bootstrap + OC-1/OC-2/OC-3 + OC-A through OC-F) | 96KB+ |
| `_patterns/04-retired-stale.md` | 9 retired patterns + 1 stale candidate with retirement / stale-flag rationale | 12KB |
| `_patterns/05-recent-additions.md` | Strengthening notes + mini-audit decisions, v54 → v68 (incl. v60/v63/v66 mini-audits + v61-v68 wiki ships) | 116KB+ |
...
**Current state (post-v68 vercel-labs/zero wiki 2026-05-18):** 31 active candidates / 43 confirmed = **0.721:1 ratio** (buffer 0.229 below the 0.95:1 mini-audit trigger). **Active count trigger REACHED at v67 + maintained at v68 — v68 mini-audit zone fully met via candidate-count metric.**

**Audit triggers post-v68 (session 73++ / 2026-05-18). Next triggers:**
- Active candidate count ≥30 trigger **REACHED at v67 + maintained at v68 (31 candidates)** — mini-audit zone fully met
- v69 wiki natural cadence (+1 wiki from v68)
- Ratio >0.95:1 mini-audit / >1.05:1 full audit (buffer 0.229; ratio-trigger unlikely soon)
- **v68 audit batch ~20 items = LARGEST in corpus history** (largest single audit batch)

**Stale-check schedule post-v68:**
- **v68 audit batch (LARGEST in corpus history, ~20 items):** Pattern #74 + #75 + #76 (v63 carry) + #77 + #78 78a/78b + #79 + #80 + **#81 sub-typology decision (81a CI-enforced vs 81b discipline-documented-but-MANUAL)** + #82 + **#83 PROMOTION** (v64+v65+v67+v68 = N=4 consolidated evidence with HIGH confidence verdict; sub-taxonomy 83a/83b/83c/83d DEFER to v69) + **#84 REFINEMENT** (84a tool-tolerance / 84b usage-enforcement decision) + #85 + **NEW from v68: Pattern #86 T1 sub-archetype #7 stale-check + Pattern #87 Skill-As-Binary-Bootstrap stale-check + OC-A through OC-F (6 observational candidates) stale-check** + Pattern #18 shared-backend-service sub-archetype (one-backend-many-IDENTITIES vs one-backend-many-PLATFORMS sister-observation evaluation) + **OC-1 Adversarial-Detection-Tolerant Architecture + OC-2 Issue-As-Enforcement-Signal Pipeline + OC-3 Quota-Arbitrage-Across-Vendor-Internal-Surfaces + Library-vocabulary item #11 Engagement-Deficit-Extreme evaluation** + AI-Generated-Repo Artifact Contamination observational
- **v69:** Pattern #52 sustained-velocity test (mattpocock + codex-plugin-cc + karpathy-skills + agentmemory + opencode-antigravity-auth + **zero**) + Pattern #83 sub-taxonomy decision (83a security / 83b methodology / 83c legal-operational / 83d **NEW from v68: pre-1.0-instability disclosure**) + **Tier-Taxonomy Review T6 consideration** (deferred from v66 Decision 14; **v68 ELEVATED PRIORITY** — NEW T1 sub-archetype #7 programming-language registration is borderline-T6 candidate)
